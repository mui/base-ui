...
- `docs/src/app/(docs)/react/page.mdx` is the combined React docs index used to build the sitemap and navigation. Its section headings and page list order, titles, and tags (for example, `[New]`, `[Preview]`) can be edited, but the outline content below the autogenerated marker should not be manually changed. Run `pnpm docs:build`, `pnpm docs:validate`, or `pnpm docs:dev` (on page visit) to regenerate it.
