...
- `docs/src/app/(docs)/react/page.mdx` is the partially autogenerated combined React docs index used to build the sitemap and navigation. Follow its inline comments when editing it. The file is regenerated when running `pnpm docs:dev` (on page visit), `pnpm docs:build`, or `pnpm docs:validate`.
