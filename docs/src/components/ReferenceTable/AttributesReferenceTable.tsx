...
export async function AttributesReferenceTable({ data, ...props }: AttributesReferenceTableProps) {
  return (
    <Table.Root {...props}>
      <Table.Head>
        <Table.Row>
          <Table.ColumnHeader className="xs:w-48 w-full sm:w-56 md:w-1/3">
            Attribute
          </Table.ColumnHeader>
          <Table.ColumnHeader className="xs:w-2/3 w-10">
          ...
          return (
            <Table.Row key={name}>
              ...
              <Table.Cell colSpan={2}>
                <div className="xs:contents hidden">
                  <AttributeDescription />
                </div>
                <div className="xs:hidden contents">
