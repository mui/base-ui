...
The `ref` prop is not handled by `mergeProps`, but refs are handled when the ref is contained within the `props` object passed to `useRender`.

`mergeProps` merges objects from left to right, so that subsequent objects' propertes in the arguments overwrite previous ones' properties.

Merging props is useful in custom components, as well as the callback version of the `render` prop for any Base UI component.
