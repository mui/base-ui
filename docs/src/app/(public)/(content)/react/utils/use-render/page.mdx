...
If it makes sense to avoid some of the work done by `useRender`, you can use it's lazy variant: `useRenderLazy`.

It's API is identical, except that it returns a callable function to render the element:

```tsx
const renderElement = useRenderLazy({
  ref: [forwardedRef, internalRef],
  props,
  render,
});
