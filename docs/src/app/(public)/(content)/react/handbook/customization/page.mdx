...
Change events such as `onOpenChange`, `onValueChange`, and `onPressedChange` are custom to Base UI rather than the DOM.
...
To leave a component uncontrolled, the event can be canceled with the `cancel()` method:
...
<NavigationMenu.Link
  onClick={(event) => {
    // Don't close the menu when clicking this link.
...
Most Base UI components can be controlled with your own external state.

Components are uncontrolled by default, meaning that they will manage their own state internally.

```tsx title="Uncontrolled mode"
<Dialog.Root>
  <Dialog.Trigger /> {/* Opens the dialog when clicked. */}
</Dialog.Root>
```

A component can be made controlled by passing external state to a prop like `open` and the state's setter to its corresponding change handler.
...
```tsx title="Opening the dialog after 1 second"
