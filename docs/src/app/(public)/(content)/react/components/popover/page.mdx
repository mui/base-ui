...
Inside the `Viewport`, the content is further wrapped in `div`s with data attributes to help with styling:

- `data-current`: The currently visible content.
- `data-previous`: The outgoing content during a transition.
- `data-next`: The incoming content during a transition.
