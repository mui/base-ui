...
To animate dimensions, target the `width` and `height` properties of the **Popup**.

The popover also supports transitions between different content.
Wrap the content you wish to transition in the `Viewport` part and apply transition styles to it.
The `Viewport` is only necessary for content animations.
It renders a `div` with a `data-activationdirection` attribute set to `left`, `right`, `down`, or `up`, indicating the new trigger's position relative to the previous one.
This allows for direction-aware transitions.

<Demo path="./demos/detached-triggers-full" />
**Popup's** `width` and `height` properties to animate dimensions.
...
It renders an element (`div` by default) with a `data-activationdirection` attribute set to `left` | `right` | `down` | `up` and a children divs marked with `data-current` (when no transition is active) or `data-next` and `data-previous`.
The `data-activationdirection` attribute specifies the position of the new trigger relative to the previous one, which can be used to apply different styles based on the direction of the transition.
