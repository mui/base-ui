...
To open a dialog from a menu item, nest the `<Dialog.Root>` inside the `<Menu.Popup>` and compose the `<Dialog.Trigger>` with the `<Menu.Item>` using the `render` prop.

```tsx {11-20} {12}#strong title="Connecting a dialog to a menu"
  ...
  <Menu.Portal>
    <Menu.Positioner>
      <Menu.Popup>
        ...
        <Dialog.Root>
          <Menu.Item render={<Dialog.Trigger />}>Dialog trigger menu item</Menu.Item>
          <Dialog.Portal>
            <Dialog.Backdrop />
            <Dialog.Popup>
              {/* prettier-ignore */}
