...
- **Avoid for critical information**: Tooltips are a great way to provide additional information about an element for sighted desktop users, but they should be used to provide supplementary information, not critical information. They do not show for touch input (i.e. mobile devices). Opt for a `Popover` on an icon button near the tool for critical information instead.
- **The trigger must be labeled**: Make sure the trigger element has an accessible name — this can be the button's text, an `aria-label`, or an `aria-labelledby` attribute. The label is the primary way for screen readers to communicate the trigger/tool's purpose.
