...
Autocomplete infers the item type from the `items` prop passed to `Autocomplete.Root`. If using `itemToStringValue`, `Autocomplete.Item`'s `value` prop must match the type of an item in the `items` array.
