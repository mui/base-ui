...
- **Avoid using as a filterable Select**: The Autocomplete's input can contain free-form text, with suggestions attached to the input to _optionally_ autocomplete the text. Use [Combobox](/react/components/combobox) instead of Autocomplete if the selection must be predefined and cannot be changed from the given list.
