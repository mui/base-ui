...
export default function ExampleGroupAutocomplete() {
  return (
    <Autocomplete.Root items={groupedTags} itemToValue={(item) => item.label}>
      ...
      <Autocomplete.Portal>
        <Autocomplete.Positioner className="outline-none" sideOffset={4}>
          <Autocomplete.Popup className="box-border w-[var(--anchor-width)] max-h-[min(var(--available-height),22.5rem)] max-w-[var(--available-width)] overflow-hidden rounded-lg bg-[canvas] text-gray-900 outline-1 outline-gray-200 shadow-lg shadow-gray-200 dark:outline-gray-300 dark:shadow-none">
            <Autocomplete.Empty className="px-4 py-2 text-[0.925rem] leading-4 text-gray-600 empty:m-0 empty:p-0">
              No tags found.
            </Autocomplete.Empty>
            <Autocomplete.List className="box-border overflow-auto scroll-pt-10 scroll-pb-[0.35rem] overscroll-contain max-h-[min(22.5rem,var(--available-height))]">
              {(group: TagGroup) => (
