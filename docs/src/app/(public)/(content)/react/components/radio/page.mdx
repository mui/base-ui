...
- **Do not wrap in a `<label>`:** The radio renders both a `<button>` and an `<input>`, which makes wrapping it in a `<label>` invalid HTML. Instead, keep the `<label>` separate and associate it using `htmlFor`, or use a [`<Field>`](/react/components/field) with a `<Field.Label>` to provide the accessible name.
