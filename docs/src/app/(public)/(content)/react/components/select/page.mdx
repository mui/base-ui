...
### Rendering a custom label in the trigger

The `Select.Value` component can accept a function as its child that is called with the current `value`, which enables you to render a custom label in the trigger based on the value.

<Demo path="./demos/label" compact />
