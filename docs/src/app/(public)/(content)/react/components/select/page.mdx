...
- **Special positioning behavior**: The select menu by default overlaps its trigger so the selected item's text is aligned with the trigger's value text. This behavior [can be disabled or customized](/react/components/select#positioning).
...
The prop makes the select menu overlap the trigger so the selected item's text is aligned with the trigger's value text.

To prevent the select menu from overlapping its trigger, set the `alignItemWithTrigger` prop to `false`.
