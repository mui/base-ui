...
The `Select.Value` component can accept a function as its child that is called with the current `value` to format it.

```jsx title="Formatting the value" "getLabel"
function getLabel(value: string | null) {
  return value && valueLabels[value];
}
