...
- **Interaction type dependent**: The `alignItemWithTrigger` positioning mode is only active if `mouse` or `keyboard` was the input modality used to open the popup. It does not apply for `touch` or `pen` input.
