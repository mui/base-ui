...
Combobox infers the item type from the `defaultValue` or `value` props passed to `<Combobox.Root>`. The type of items held in the `items` array must also match the type passed to `<Combobox.Item>`'s `value` prop.
