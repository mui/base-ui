...
export default function ExampleCombobox() {
  ...
  function handlePopoverOpenChangeComplete(open: boolean) {
    if (!open) {
      setInputValue('');
    }
  }
  ...
  return (
    <Popover.Root
      ...
      onOpenChangeComplete={handlePopoverOpenChangeComplete}
    >
          ...
          <Popover.Popup className={styles.Popup} aria-label="Select country">
            ...
            >
              ...
              <Combobox.Status className={styles.NoResults}>
                {filteredCountries.length === 0 ? 'No countries found' : null}
              </Combobox.Status>
            </Combobox.Root>
          </Popover.Popup>
        </Popover.Positioner>
