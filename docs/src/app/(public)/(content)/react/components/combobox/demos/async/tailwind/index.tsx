...
export default function ExampleAsyncCombobox() {
  ...
  function getStatus(): React.ReactNode {
    ...
    return `Showing ${searchResults.length} result${searchResults.length === 1 ? '' : 's'}`;
  }
      ...
      <Combobox.Portal>
        <Combobox.Positioner className="outline-none" sideOffset={4}>
          ...
          >
            ...
            <Combobox.Empty className="px-4 py-2 text-[0.925rem] leading-4 text-gray-600 empty:hidden">
              {getEmptyMessage()}
            </Combobox.Empty>
            <Combobox.List>
              {(user: DirectoryUser) => (
