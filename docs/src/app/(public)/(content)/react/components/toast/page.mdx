...
- It's generally recommended to wrap `<Toast.Provider>` around your entire app. All toasts will be rendered in the same viewport.
- `<Toast.Viewport>` is positioned with your own CSS. The example above specifies `data-position` attributes for the CSS to adjust the position of the viewport container—the available positions in the example are `top`, `top-left`, `top-right`, `bottom`, `bottom-left`, and `bottom-right`, but you're free to change the CSS as needed.
...
<PropsReferenceTable
  ...
  data={{
    toasts: {
      type: 'Toast[]',
      description: 'The array of toasts.',
    },
    add: {
      type: '(options: useToast.AddOptions) => void',
      description: 'Add a toast to the toast list.',
    },
    remove: {
      type: '(toastId: string) => void',
      description: 'Remove a toast from the toast list.',
    },
    update: {
    ...
    promise: {
      type: '<Value>(promise: Promise<Value>, options: useToast.PromiseOptions) => void',
      description:
...
<PropsReferenceTable
  data={{
    ...
    onRemove: {
      type: '() => void',
      description: 'A callback invoked when the toast is removed.',
    },
    onRemoveComplete: {
      type: '() => void',
      description:
        'A callback invoked when the toast is removed after any animations are complete.',
    },
    actionProps: {
...
The `add` method returns a toast ID, which can be used to update the toast later.
...
The `add` method returns a toast ID, which can be used to remove the toast from the toast list later.
...
Creates an asynchronous toast that resolves with a value, with three possible states for the toast: `loading`, `success`, and `error`.
...
const toastId = toast.promise(
  ...
  {
    // Each are a shortcut for the title of the toast.
    loading: 'Loading...',
...
An asynchronous toast can be created with three possible states for the toast's title: `loading`, `success`, and `error`. The `type` string matches these states to change the styling. Each of the states also accepts the [method options](/react/components/toast#method-options) object for more granular control.

<Demo path="./demos/promise" />

### Custom

A toast with custom data can be created by passing any typed object interface to the `data` option. This enables you to pass any data you need to the toast and access it in the toast's rendering logic.
