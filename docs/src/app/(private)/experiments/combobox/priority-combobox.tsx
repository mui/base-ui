...
function CustomCombobox(props: { items: Priority[] }) {
  return (
    <Combobox.Root items={props.items} defaultValue={props.items?.[0]} autoHighlight>
      <Combobox.Trigger className="h-8 cursor-default items-center justify-between rounded-lg border border-gray-200 bg-[canvas] bg-clip-padding px-3 text-sm text-gray-900 select-none hover:bg-gray-50 focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-800 data-[popup-open]:bg-gray-50">
        <Combobox.Value>
          {(priority: Priority) => (
      ...
      <Combobox.Portal>
        <Combobox.Positioner align="start" sideOffset={4} trackAnchor={false}>
          ...
          >
            <div className="grid w-60 grid-cols-[1fr_1.4rem] gap-2 p-1 px-2 text-center">
              <Combobox.Input
                placeholder="Set priority to..."
                className="col-start-1 h-8 w-full pl-3.5 text-sm font-normal text-gray-900 focus:outline focus:outline-0"
              />
              <div className="align-self-center grid-col-start-2 flex h-[1.4rem] justify-center self-center rounded-md border border-1 border-gray-300 text-xs text-gray-800">
                P
              </div>
            </div>
            ...
            <Combobox.List className="max-h-[min(calc(24rem-var(--input-container-height)),calc(var(--available-height)-var(--input-container-height)))] scroll-py-2 overflow-y-auto overscroll-contain py-2 empty:p-0">
              {(priority: Priority) => (
                ...
                >
                  ...
                  <div className="col-start-4 text-center text-xs text-gray-600">
                    {priority.code}
                  </div>
                </Combobox.Item>
              )}
