...
export default function ExamplePopoverCombobox() {
  return (
    <Combobox.Root items={priorities} defaultValue={priorities[0]}>
      <Combobox.Trigger className="flex bg-[canvas] h-10 min-w-[8rem] items-center justify-between gap-3 rounded-md border border-gray-200 pr-3 pl-3.5 text-base text-gray-900 select-none hover:bg-gray-100 focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-800 data-[popup-open]:bg-gray-100 cursor-default">
        <Combobox.Value>
          {(priority: Priority) => (
            ...
            <Combobox.List className="overflow-y-auto scroll-py-2 py-2 overscroll-contain max-h-[min(calc(24rem-var(--input-container-height)),calc(var(--available-height)-var(--input-container-height)))] empty:p-0">
              {(priority: Priority) => (
                ...
                >
                  ...
                  <div className="col-start-4 text-center">{priority.code}</div>
                </Combobox.Item>
              )}
            </Combobox.List>
          </Combobox.Popup>
