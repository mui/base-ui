...
Popups that open when hovering an info icon should use [Popover](/react/popover) with the `openOnHover` prop on the trigger instead of a tooltip. This way, touch users and screen reader users can access the content.
...
Use the Toast component's [anchoring ability](/react/toast#anchored-toasts) for more ergonomic DX, to ensure the message is announced to screen readers, and to support complex content.
