...
- **Form controls must have an accessible name**: It can be created using `<label>` elements, or the `Field` and `Fieldset` components. See [Labeling a checkbox group](#labeling-a-checkbox-group) and the [forms guide](/react/forms).

## Anatomy

Checkbox Group is composed together with [Checkbox](/react/checkbox). Import the components and place them together:
...
Use [Field](/react/field) and [Fieldset](/react/fieldset) for group labeling and form integration:
...
The group controls the parent checkbox's [indeterminate](/react/checkbox#CheckboxRoot-indeterminate) state when some, but not all, child checkboxes are checked.
