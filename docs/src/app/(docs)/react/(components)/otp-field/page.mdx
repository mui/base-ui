...
- **Form controls must have an accessible name**: It can be created using a `<label>` element or the `Field` component. See [Labeling an OTP field](#labeling-an-otp-field) and the [forms guide](/react/forms).
...
Use [Field](/react/field) to handle label associations and form integration:
