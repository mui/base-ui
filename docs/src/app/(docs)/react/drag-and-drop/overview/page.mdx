...
Base UI includes headless drag-and-drop for making elements draggable and building drop zones. It works across mouse, touch, and pen, with pointer-appropriate defaults for starting a drag (see [Activation](/react/components/draggable#activation)). The components are unstyled, including the preview that follows the pointer.
...
A `createKind` call has a unique identity. Declare it once and share the returned value with every draggable and target in the interaction. Two separate `createKind('card')` calls do not match. The name is only a debugging aid.
...
Drop a node on a folder, on the open grid, or on an ancestor in the breadcrumb to move it there. Plain <kbd>Space</kbd> or <kbd>Enter</kbd> opens a focused folder. [`canDrop`](/react/components/drop-target#nested-drop-targets) refuses a folder dropped into its own subtree.
