...
Cards can be placed at absolute coordinates and dropped anywhere. Keep the preview's `offset` at its `'source'` default to preserve the pickup point. Use `restrictToElement` to keep the drag inside the canvas, then read the new position from `getSnappedLocalPoint({ anchor: 'source' })`. Because the canvas declares no snap steps, the card stays where the preview is released. Moving it to the end of the list places it above overlapping cards.

<DemoFigmaBoard />

### Kanban board

The Kanban example renders an empty card at the resolved insertion point. `useDragMonitor` reads the pointer during the drag, finds the closest column and slot, and inserts the placeholder there. The surrounding cards make room before the drop, including when the pointer is between columns.
