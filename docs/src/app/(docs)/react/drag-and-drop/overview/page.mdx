...
Drop a node on a folder, on the open grid, or on an ancestor in the breadcrumb to move it there. Hovering a folder mid-drag opens it, and [`canDrop`](/react/components/drop-target#nested-drop-targets) refuses a folder dropped into its own subtree.
