...
Use it with [`keyboardActivation="manual"`](/react/components/draggable#moving-the-pickup-elsewhere) when the element needs <kbd>Space</kbd> for another action. For example, a "Reorder" menu item can start the drag after <kbd>Space</kbd> opens the menu.
