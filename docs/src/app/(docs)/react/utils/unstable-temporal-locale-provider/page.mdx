# Temporal Locale Provider

<Subtitle>Defines the locale to use in the temporal components.</Subtitle>
<Meta
  name="description"
  content="A temporal locale provider component that defines the locale to use in the temporal components."
/>

import { DemoTemporalLocaleProviderHero } from './demos/hero';

<DemoTemporalLocaleProviderHero />

## API reference

Import the component plus the `date-fns` library locale and wrap them around your app.

If every temporal component in your app shares the same locale, you should use the provider once at the root of your application.

```jsx title="Anatomy"
import { fr } from 'date-fns/locale/fr';
import { UnstableTemporalLocaleProvider as TemporalLocaleProvider } from '@base-ui/react/temporal-locale-provider';

<TemporalLocaleProvider locale={fr}>
  {/* Your app or a group of components */}
</TemporalLocaleProvider>;
```

<Reference component="TemporalLocaleProvider" />

## Examples

### Nesting locales

The `<TemporalLocaleProvider>` can be nested to change the locale for a part of the application.

This could be useful when you need some temporal components in your application to use a different locale than the rest of your app.

import { DemoTemporalLocaleProviderNesting } from './demos/nesting';

<DemoTemporalLocaleProviderNesting />

export const metadata = {
  keywords: ['Localization', 'Locale', 'Temporal', 'Date Fns', 'Base UI'],
};
