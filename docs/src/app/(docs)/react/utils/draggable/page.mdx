...
<Draggable.Root
  ...
  onMoveEnd={({ source, target }) => {
    try {
      if (target && column.matches(target)) {
...
The following example wraps `<Draggable.Root>` for items of any kind. `Draggable.Root.Props<Payload>` keeps `payload` required whenever the kind declares one:

```tsx title="A wrapper for items of any kind"
import * as React from 'react';
import { Draggable } from '@base-ui/react/draggable';

export function SortableItem<Payload>(props: Draggable.Root.Props<Payload>): React.JSX.Element {
  return <Draggable.Root {...props} />;
}
```

A wrapper that destructures its props can't forward the rest as is: while `Payload` is generic, TypeScript can't tell whether `payload` is required. Declare `kind` and `payload` explicitly in that case:
