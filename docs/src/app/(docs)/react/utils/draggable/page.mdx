...
To observe every drag on the page rather than one source, use [`useDragMonitor`](#usedragmonitor).
...
The page doesn't scroll during a drag by default. Since `<Draggable.Viewport>` renders an element, register the document with [`useDragDropManager`](#usedragdropmanager) instead. It accepts the same options as the viewport part:
...
Both `source` and `target` records let handlers store data for later handlers to read. Declare its type as the second type argument of `createKind`, and set it with `updateDragData`. It starts as `undefined` on each drag. For example, record where the item was grabbed when the drag starts, and read it on every move:
...
Creates a kind identified by a string key instead of a unique value. Use it only when independently bundled parts of an application can't share the value returned by `createKind`. The same key identifies the same kind across bundles and hot reloads. Prefix the key with your app or package name, and use one payload type per key, since TypeScript can't check that two bundles agree.

```tsx title="A kind shared across bundles"
...
Observes every drag on the page, wherever it started. A monitor has no element and needs no `<Draggable.Provider>`, so a status bar in the application shell can react to drags happening anywhere. Use it for status indicators, analytics, or to commit drops from one place instead of from each target.

In the shape sorter below, the cutouts only declare what they accept. A monitor updates the status text and places each shape after a drop.

import { DemoMonitorShapeSorter } from './demos/use-drag-monitor/hero';

<DemoMonitorShapeSorter />

Pass `accept` to observe only some [kinds](#kinds-and-payloads). It types `source.payload` the same way as a target's `accept`. Omit it to observe every drag with an `unknown` payload.
...
The handlers are the same as on `<Draggable.Root>`, minus `onBeforeMoveStart`. A monitor mounted during a drag observes the rest of it, so `onMoveEnd` can fire without a preceding `onMoveStart`.

<TypesDraggable.useDragMonitor hideDescription />

## useDragDropManager

Returns the page-wide drag manager, for registering elements without rendering the Draggable parts. Use it to make an existing element draggable, to integrate a widget React doesn't render, or to scroll the page. It requires a `<Draggable.Provider>` above the component calling it.

The shape sorter below registers its pieces, cutouts, and monitor through the manager instead of `<Draggable.Root>`, `<Draggable.Target>`, and `useDragMonitor`.

import { DemoEngineShapeSorter } from './demos/use-drag-drop-manager/hero';

<DemoEngineShapeSorter />

Every call returns the same manager. Calling the hook in two places doesn't isolate them, and `cancelDrag()` ends whichever drag is active. Use distinct kinds to keep unrelated features apart.

```tsx title="Getting the manager"
const manager = Draggable.useDragDropManager();
```

The registration methods take an element and a function returning its options, and return a cleanup function. Register from an effect and return the cleanup. The methods are stable, so they're safe to list as dependencies:
...
Base UI reads the options function at different times depending on the option:
...
React.useEffect(() => {
  ...
  return manager.registerAutoScroller(scrollElement, () => ({ maxSpeed: 600 }));
}, [manager, gridApi]);
```

### registerMonitor

Registers a monitor, with the options of [`useDragMonitor`](#usedragmonitor). It takes only the options function:
