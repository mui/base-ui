...
Drag sources and drop targets both fire event handlers as a drag progresses. Every handler receives a value first, with the drag `source` and the `target` it is over, and an `eventDetails` object second with the event `reason`, the native `event`, and the drag `location`.

When extracting a handler, use the types on its component namespace, such as `Draggable.Root.MoveStartValue<Payload>`, `Draggable.Root.MoveStartEventDetails`, and `Draggable.Root.MoveStartEventReason`. Target handlers expose corresponding types such as `Draggable.Target.DropValue<SourcePayload, TargetPayload>` and `Draggable.Target.DropEventDetails`. Value types also accept drag-data type arguments when needed.
...
Commit a successful drop in `onMoveEnd`. There, `target` is the target that received the drop, or `null` when the drag was canceled or released elsewhere, and `eventDetails.reason` says why the drag ended. When the handler also clears temporary state, use `try/finally` so the cleanup runs even if committing throws:
...
Every handler receives the drag `source` and `target` in its first argument, and a `location` history in `eventDetails`:
