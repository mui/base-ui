...
Both `source` and `target` records let handlers store data for later handlers to read. Declare its type as the second type argument of `createKind`, and set it with `updateDragData`. It starts as `undefined` on each drag unless the source supplies `getDragData`. This callback runs once after activation is accepted, before drop targets resolve and custom preview content renders. Its result stays available across source rerenders. For example:
...
<Draggable.Root
  kind={card}
  payload={id}
  getDragData={({ element, input }) => ({
    grabOffsetX: input.clientX - element.getBoundingClientRect().left,
  })}
  onMove={({ source }) => {
