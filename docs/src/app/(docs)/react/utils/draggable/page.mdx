...
The following example wraps `<Draggable.Root>` for items of any kind. Inside a generic component, `Draggable.Root.Props<Payload>` can't tell whether `Payload` is `undefined`, so declare `kind` and `payload` explicitly to keep them required and checked against each other:
...
export function SortableItem<Payload>(
  props: Draggable.Root.Props<Payload> & { kind: Draggable.Kind<Payload>; payload: Payload },
): React.JSX.Element {
  return <Draggable.Root {...props} className="Item" />;
}
```
