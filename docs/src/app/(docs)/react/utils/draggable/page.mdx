...
The key must contain a `/`. Use your app or package name as the prefix, since keys are shared by the whole page and a bare name like `'card'` could collide with another library. Both sides must also agree on the payload type: TypeScript can't check that across bundles, so a mismatch surfaces at runtime.
