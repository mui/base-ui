...
Drag sources and drop targets both fire event handlers as a drag progresses. Every handler receives a value first, with the drag `source` and the `target` it is over, and an `eventDetails` object second with the event `reason`, the native `event`, and the drag `location`. `onBeforeMoveStart` is the exception: it runs before any target is resolved, so its value holds only the `source`, and its `eventDetails` carry the pointer `input` and `cancel()`.

When extracting a handler, use the types on its component namespace, such as `Draggable.Root.MoveStartValue<Payload>`, `Draggable.Root.MoveStartEventDetails`, and `Draggable.Root.MoveStartEventReason`. Target handlers expose corresponding types such as `Draggable.Target.DropValue<SourcePayload, TargetPayload>` and `Draggable.Target.DropEventDetails`. Value types also accept drag-data type arguments when needed. Monitor and `registerSource` handlers use the `Draggable.Root` types, and `registerTarget` handlers use the `Draggable.Target` ones.
...
<Draggable.Root
  ...
  onTargetChange={({ target }) => console.log('over', target?.payload ?? 'nothing')}
  // Fires once when the drag ends, whether it was dropped, released
  // outside a target, or canceled.
  onMoveEnd={({ target }) => {
    if (target) {
      console.log('dropped on', target.payload);
    }
  }}
/>
```

Commit a successful drop in `onMoveEnd`. There, `target` is the target that received the drop, or `null` when the drag was canceled or released elsewhere. `eventDetails.canceled` tells a cancel, such as <kbd>Esc</kbd> or `cancelDrag()`, from a release, and `eventDetails.reason` says exactly why the drag ended. A source can land on targets of any kind, so `target.payload` is `unknown` until you narrow it with the target's [kind](#identifying-a-target). When the handler also clears temporary state, use `try/finally` so the cleanup runs even if committing throws:
...
<Draggable.Root
  ...
  onMoveEnd={({ source, target }) => {
    try {
      if (target && columnKind.matches(target)) {
        commitMove(source.payload, target.payload);
      }
    } finally {
      clearDragState();
    }
...
`eventDetails.reason` is one of the following. Handle reasons you don't recognize too, since more cancel reasons may be added:

- `'drop'`: the item was released over a drop target that accepted it, or over its own slot in a collision provider.
- `'outside-release'`: the item was released outside any accepting drop target.
- `'escape-key'`, `'tab-key'`: the user pressed <kbd>Esc</kbd> or <kbd>Tab</kbd>.
...
- `'handler-error'`: one of your handlers threw. Base UI cancels the drag and rethrows the error separately.
- `'document-detached'`: the document was removed, for example when an iframe closed.

Apart from `onBeforeMoveStart`, every handler receives the drag `source` and `target` in its first argument, and a `location` history in `eventDetails`:
...
`<Draggable.Target>` fires the handlers below. Their first argument holds the drag `source` and the target's own record as `target`, and `eventDetails` holds the same `location` history:
...
Pass `payload` when other code must identify the target, for instance a source or monitor handler reading the `target` it receives. The value is available as `target.payload` there and in the target's own handlers:
...
React.useEffect(
  () =>
    manager.registerViewport(document.documentElement, () => ({
...
Assert on what a user could observe: the data attributes on the source and targets, and the arguments your handlers received. Handlers take two arguments, so check `mock.calls[0][0]` for the value and `mock.calls[0][1]` for the details rather than `toHaveBeenCalledWith` with a single argument. A successful drop has the `'drop'` reason:

```tsx title="Asserting on a drop"
expect(onMoveEnd).toHaveBeenCalledTimes(1);
expect(onMoveEnd.mock.calls[0][0].target?.payload).toBe('done');
...
<Draggable.Root
  ...
  onBeforeMoveStart={({ source }, eventDetails) => {
    ...
    source.updateDragData({ grabOffsetX: eventDetails.input.clientX - rect.left });
  }}
  onMove={({ source }) => {
...
const snapToRows: Draggable.Root.Modifier = ({ point }) => ({
  x: point.x,
...
const nudgeRight: Draggable.Root.Modifier = ({ point, scale }) => ({
...
const snapToAngle: Draggable.Root.Modifier = ({ point, shiftKey, initialPoint }) => {
  if (!shiftKey) {
    return point;
  }
...
The source receives `[data-settling]` while the clone settles, so it can act as an empty placeholder until the clone arrives:
...
Update the order in `onMoveEnd`. The `target.payload` it receives identifies the item under the pointer, and `target.getLocalPoint()` tells which half of it the pointer is in. For a vertical list, a `y` above `0.5` means the bottom half. `target` is `null` when the drag was canceled, released outside the group, or released over the dragged item itself.

import { DemoSortableOnDrop } from './demos/sortable-on-drop';

<DemoSortableOnDrop />

To show an insertion indicator, compute the position in `onCollisionChange` and store it in state. Its `eventDetails` include the `previousTarget`, so you can skip updates when the position hasn't changed:
...
<Draggable.CollisionProvider
  kind={task}
  onCollisionChange={({ target }, eventDetails) => {
    const next = getPosition(target);
    const previous = getPosition(eventDetails.previousTarget);
  ...
  onMoveEnd={({ source, target }) => {
    setPosition(null);
    if (target) {
      moveTask(source.payload, getPosition(target));
    }
  }}
/>;
```
...
To move items as the drag goes, update the order in `onCollisionChange` instead of `onMoveEnd`. Compare `location.current.input` with `location.previous.input`, both on `eventDetails`, to place the item according to the pointer's direction.

import { DemoSortableLive } from './demos/sortable-live';

<DemoSortableLive />

Save the original order in `onMoveStart`, and restore it in `onMoveEnd` when `eventDetails.reason` isn't `'drop'`, which means the drag was canceled or released outside any target. A release over the dragged item itself has the `'drop'` reason but a `null` target. Keep the current order in that case.
...
The following example wraps `<Draggable.Root>` for items of any kind. `Draggable.Root.Props<Payload>` keeps `payload` required whenever the kind declares one. Give `Payload` a default of `undefined`, like the component, so the wrapper also works without a kind. Type it with `Draggable.Root.Props<Payload>` rather than `React.ComponentProps<typeof Draggable.Root>`, which fills `Payload` with `unknown`, a payload no kind matches:

```tsx title="A wrapper for items of any kind"
import * as React from 'react';
import { Draggable } from '@base-ui/react/draggable';

export function SortableItem<Payload = undefined>(
  props: Draggable.Root.Props<Payload>,
): React.JSX.Element {
  return <Draggable.Root {...props} />;
}
```

A wrapper that destructures its props can't forward the rest as is: while `Payload` is generic, TypeScript can't tell whether `payload` is required. Declare `kind` and `payload` explicitly in that case. Such a wrapper requires both, so pass `payload={undefined}` for a kind without a payload:

```tsx title="A wrapper that adds a handle"
export function SortableItem<Payload>({
  children,
  ...props
}: Draggable.Root.Props<Payload> & {
  kind: Draggable.Kind<Payload>;
  payload: Payload;
}): React.JSX.Element {
  return (
    <Draggable.Root {...props}>
      <Draggable.Handle aria-hidden="true">⠿</Draggable.Handle>
      {children}
    </Draggable.Root>
  );
}
...
A generic `<Draggable.Target>` wrapper declares `accept`, and `payload` when the target has one, in the same way. This applies whether it spreads or destructures its props:

```tsx title="A drop target for items of any kind"
export function Slot<Source, Payload>({
  children,
  ...props
...
The built-in [movement modifiers](#constraining-movement). Pass them to the `modifiers` prop of `<Draggable.Root>` or `<Draggable.Preview>`. To write your own, type it as `Draggable.Root.Modifier`, as shown in [Custom modifiers](#custom-modifiers).
...
Draggable.useMonitor({
  ...
  // Fires as the pointer moves, at most once per animation frame.
  onMove: (_, eventDetails) => console.log('at', eventDetails.location.current.input.clientX),
  // Fires when the drop targets under the pointer change.
  onTargetChange: ({ target }) => console.log('over', target?.payload ?? 'nothing'),
  // Fires once when the drag ends.
  onMoveEnd: ({ source, target }) => {
    if (target) {
      console.log('dropped', source.payload.title, 'on', target.payload);
    }
  },
});
```
...
React.useEffect(() => {
  return manager.registerSource(element, () => ({ kind: card, payload: id }));
}, [manager, id]);
```
...
| `disabled`, `handle`, and the styles applied while idle | At registration and on the next `pointerdown`. Re-register to apply a change at once. |

### registerSource

Registers a drag source. It accepts the options of [`<Draggable.Root>`](#root), plus `handle` to restrict pickup to an element, and `preview` to configure the preview:

```tsx title="Registering a source"
manager.registerSource(element, () => ({
  kind: card,
  payload: id,
  handle: handleRef,
  preview: { offset: 'pointer' },
}));
```

`preview` takes the options of [`<Draggable.Preview>`](#preview). Pass a `render` function instead of children to show custom content:
...
TypeScript infers `source.payload` from `accept` but can't infer the target's own `payload` through the options function. Pass both type arguments, as in `registerTarget<typeof card, SlotPayload>`, when the target reads `target.payload`.
...
React.useEffect(() => {
  ...
  return manager.registerViewport(scrollElement, () => ({
    maxSpeed: 600,
...
Ends the drag in progress. `onMoveEnd` fires with a `null` target and the `'imperative-action'` reason. It does nothing when no drag is active. Use it when a route change, a dialog, or a deleted record invalidates the drag:
