...
Commit a successful drop in `onMoveEnd`. There, `target` is the target that received the drop, or `null` when the drag was canceled or released elsewhere, and `eventDetails.reason` says why the drag ended. A source can land on targets of any kind, so `target.payload` is `unknown` until you narrow it with the target's [kind](#identifying-a-target). When the handler also clears temporary state, use `try/finally` so the cleanup runs even if committing throws:
...
const snapToRows: Draggable.DragModifier = ({ point }) => ({
  x: point.x,
...
const nudgeRight: Draggable.DragModifier = ({ point, scale }) => ({
...
const snapToAngle: Draggable.DragModifier = ({ point, shiftKey, initialPoint }) => {
  if (!shiftKey) {
    return point;
  }
...
export function SortableItem<Payload>(
  props: Draggable.Root.Props<Payload> & { kind: Draggable.DragKind<Payload>; payload: Payload },
): React.JSX.Element {
