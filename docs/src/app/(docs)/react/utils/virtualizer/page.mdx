...
A grouped collection is an array of groups, each an object with an `items` array, as for the list components: a collection whose elements carry an `items` array is taken for a grouped one. Pass it as `items` together with `renderGroupHeader`, which renders the element carrying a group's name. Inside `<Combobox.List>` that element is `<Combobox.GroupLabel>`; the virtualizer wraps each group's rendered items in a `role="group"` element labelled by it, so `<Combobox.Group>` is not used. The virtualizer only knows a group by its `items`, so annotate the group parameter with your own group type to read anything else from it:
