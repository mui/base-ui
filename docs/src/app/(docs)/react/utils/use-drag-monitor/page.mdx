...
Use [`Draggable.useActiveDrag`](/react/components/draggable#useactivedrag) to read the active source without event handlers. It re-renders only when a drag starts or ends. For imperative registration, obtain the manager with [`useDragDropManager`](/react/utils/use-drag-drop-manager#monitor-drags) in a React component, then call its `registerMonitor` method.
