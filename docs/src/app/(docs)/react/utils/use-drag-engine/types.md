...
[//]: types.ts '<-- Autogenerated By (do not edit the following markdown directly), run: pnpm docs:validate "(docs)/react/utils/use-drag-engine"'
...
**useDragEngine Return Value:**

```tsx
type ReturnValue = UseDragEngineReturnValue;
```

### useDragEngine.ReturnValue

```typescript
type useDragEngineReturnValue = {
  /**
  ...
  registerDraggable:
    ...
    | ((
        element: HTMLElement,
        getParameters: () => Omit<RegisterDraggableParameters, 'payload'>,
      ) => DragCleanupFn);
  /**
...
### UseDragEngineReturnValue

The drag engine's imperative API returned by [`useDragEngine`](#usedragengine):
