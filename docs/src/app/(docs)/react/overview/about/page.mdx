...
The stub modules export an empty script string, so the prehydration scripts are excluded from the bundle—the same outcome modern bundlers get from the `browser` condition.
