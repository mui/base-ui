...
- **Prefer Combobox for large or remote data**: Use [Combobox](/react/components/combobox) when the input is the control or the list is large, virtualized, or asynchronously loaded. Use [Autocomplete](/react/components/autocomplete) when users can enter values that are not in the list.
