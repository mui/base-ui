...
Place `<Checkbox.PaintSelectionProvider>` inside `<CheckboxGroup>`, around the checkboxes, to enable paint selection.
...
The provider is optional; omitting it excludes the paint controller from the bundle.
