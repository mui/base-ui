...
- **Prefer using tooltips as visual labels only**: Tooltips should act as supplementary visual labels for sighted mouse users. Tooltips alone are not accessible to touch or screen reader users. See [Alternatives to tooltips](#alternatives-to-tooltips) for more details.
...
Tooltips should be supplementary, visual-only popups that provide clarity in high-density UIs. Omitting a tooltip from a user should not result in loss of critical information.

Tooltips do not work when using a touch pointer. Unlike mouse pointers with hover capability, there is no easy-to-discover way to reveal a tooltip before tapping its trigger when using a touch pointer.

iOS provides no tooltips in its operating system, while Android may show a tooltip on long press. However, on the web, long press is often used to trigger contextual menus in the browser, which can lead to potential conflicts. For this reason, tooltips are disabled on touch devices by default.

### Infotips

Popups that open when hovering an info icon should use [Popover](/react/components/popover) and the `openOnHover` prop on the trigger instead. This way, touch users and screen reader users can access the content.
...
In the vast majority of cases, tooltips should act as labels, not descriptions. If you need to provide a description about an element, use inline text or a [Popover](/react/components/popover) trigger near the tooltip trigger if space is limited.

Attempting to hide additional, important information behind a tooltip excludes touch and screen reader users — popovers are accessible to all users. If the description is not critical to understanding the element, a tooltip may be used to provide clarity for sighted mouse users.

### Contextual feedback messages

Use the Toast component's [anchoring ability](/react/components/toast#anchored-toasts) for more ergonomic DX, to ensure the message is announced to screen readers, and to support complex content inside.
