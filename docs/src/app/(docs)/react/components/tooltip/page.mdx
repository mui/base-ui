...
In-place changes set no `data-activation-direction`, so the demo derives the slide direction from the chapter order and styles it through a custom `data-direction` attribute.
