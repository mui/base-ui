...
```tsx title="Labeling multi-thumb range sliders" {5-6}
...
For grouped multi-thumb range sliders, [Fieldset](/react/components/fieldset) can be used by replacing the `<Fieldset.Root>` element with `<Slider.Root>` via the `render` prop. `<Fieldset.Legend>` provides the visible label for the group:
...
```tsx title="Using Slider in a form" {3}
<Form>
