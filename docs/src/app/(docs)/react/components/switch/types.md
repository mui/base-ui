...
| checkedChangeAction | `((checked: boolean, eventDetails: Switch.Root.ChangeEventDetails) => void \| PromiseLike<unknown>)` | -       | Async function that is executed when the switch is activated or deactivated.&#xA;It runs in a React Transition and updates the switch optimistically while pending. Use this when the checked change needs to perform async work, such as sending&#xA;the new checked state to a server. |
