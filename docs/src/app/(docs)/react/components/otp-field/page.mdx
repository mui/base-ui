...
Pass an `id` to `<OTPField.Root>` and use a native `<label>` with a matching `htmlFor`. Add
`aria-describedby` to `<OTPField.Root>` when you want supporting text announced with the field.
Let the first input use the field label, and add `aria-label` to the remaining inputs so assistive
technology can announce which later slot is focused:
...
Because each `<OTPField.Input>` is a real input, native `placeholder` props and CSS work as
usual. This example keeps placeholder hints visible until the active slot receives focus.
