...
Use `sanitizeValue` to normalize accepted values before state updates, such as converting
alphanumeric codes to uppercase. It runs after `validationType` filtering, and the result is checked
against `validationType` again. Use `validationType="none"` when the sanitizer should provide the
