...
`payload` attaches data to the target, read back as `self.payload` in its own callbacks. Use `getPayload` when that data must be derived from the current drag; it runs on each resolution with the same context as `canDrop`. Its type is inferred separately from the one `accept` brings: `source.payload` is the dragged item's data, `self.payload` this target's own.
