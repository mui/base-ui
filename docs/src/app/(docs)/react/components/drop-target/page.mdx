...
`DropTarget` marks where a drag can be released. It pairs with [Draggable](/react/components/draggable), which defines what can be picked up. See the [drag and drop overview](/react/drag-and-drop/overview) for examples and the [collections guide](/react/drag-and-drop/collections) for reorderable lists and boards.
