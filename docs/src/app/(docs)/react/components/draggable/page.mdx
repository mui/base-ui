...
The handle is a pointer affordance and renders a `span` by default. Keep the item itself operable through its normal keyboard interaction or an app-level shortcut; see the [accessibility guide](/react/drag-and-drop/accessibility).
...
A modifier takes the point the engine would use and returns the point to use instead. Alongside `point`, it receives where the drag began (`initialPoint`), the cursor (`input`), the source and preview rects, `scale`, and the modifier keys held by the event that produced the move. It runs on every drag frame, so keep it cheap.
