  ...
  return (
    <div className="flex flex-col items-start gap-1">
      <Select.Root defaultValue={shippingMethods[0]} itemToStringValue={(item) => item.id}>
        <Select.Label className="cursor-default text-sm leading-5 font-bold text-neutral-950 dark:text-white">
          Shipping method
        </Select.Label>
        <Select.Trigger className="flex min-h-8 min-w-[16rem] items-start justify-between gap-3 pl-3 pr-2 py-1.5 text-sm leading-5 border border-neutral-950 dark:border-white bg-white dark:bg-neutral-950 text-neutral-950 dark:text-white select-none hover:not-data-disabled:bg-neutral-100 dark:hover:not-data-disabled:bg-neutral-800 active:not-data-disabled:bg-neutral-200 dark:active:not-data-disabled:bg-neutral-700 data-disabled:border-neutral-500 data-disabled:text-neutral-500 disabled:border-neutral-500 disabled:text-neutral-500 dark:data-disabled:border-neutral-400 dark:data-disabled:text-neutral-400 data-[popup-open]:bg-neutral-100 dark:data-[popup-open]:bg-neutral-800 font-normal focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-blue-800">
          <Select.Value>
            {(method: ShippingMethod) => (
              <span className="flex flex-col items-start gap-0.5">
                <span className="text-sm leading-5">{method.name}</span>
                <span className="text-xs leading-4 text-neutral-600 dark:text-neutral-400">
              ...
              <Select.List className="relative py-1 scroll-py-6 overflow-y-auto max-h-[var(--available-height)]">
                {shippingMethods.map((method) => (
                  <Select.Item
                    key={method.id}
                    value={method}
                    className="group/item grid cursor-default grid-cols-[0.75rem_1fr] items-start gap-2 py-1.5 pr-4 pl-2.5 text-sm leading-5 outline-hidden select-none group-data-[side=none]:pr-12 [@media(hover:hover)]:[&[data-highlighted]]:bg-neutral-950 [@media(hover:hover)]:[&[data-highlighted]]:text-white dark:[@media(hover:hover)]:[&[data-highlighted]]:bg-white dark:[@media(hover:hover)]:[&[data-highlighted]]:text-neutral-950"
                  >
                    ...
                    <Select.ItemText className="col-start-2 flex flex-col gap-0.5">
                      <span className="text-sm leading-5">{method.name}</span>
                      <span className="text-xs leading-4 text-neutral-600 [@media(hover:hover)]:group-data-[highlighted]/item:text-neutral-400 dark:text-neutral-400 dark:[@media(hover:hover)]:group-data-[highlighted]/item:text-neutral-600">
                        {method.duration} ({method.price})
                      </span>
                    </Select.ItemText>
                  </Select.Item>
