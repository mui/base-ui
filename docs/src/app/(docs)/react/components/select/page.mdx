...
the list. `items` must be an array: of values, of `{ label, value }` entries, or of groups; the
object form of `items` is not supported. Use `getItemKey` when item values are objects.

A grouped collection also needs `renderGroupHeader`, returning a `<Select.GroupLabel>`. The
virtualizer wraps each group's rendered items in a `role="group"` element labeled by it, so
`<Select.Group>` is not used; see [grouped collections](/react/utils/virtualizer#grouped-collections).

```tsx title="Virtualizing a grouped select"
<Select.Root<Country> items={groupedCountries} itemToStringLabel={getCountryLabel}>
  {/* ... */}
  <Select.List>
    <Virtualizer<Country>
...
`overflow` on `<Select.List>` leaves two nested scrollers fighting each other. For the same reason,
`scroll-padding` belongs on the virtualizer: it is what keeps a row scrolled into view clear of the
scroll arrows, and on `<Select.List>` it no longer applies to anything.
