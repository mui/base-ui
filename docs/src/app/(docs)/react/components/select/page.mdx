...
- **Add filtering with FilterSelect, and prefer Combobox for large lists**: Use [FilterSelect](/react/components/filter-select) for a modest list of predefined values that needs filtering. Use [Combobox](/react/components/combobox) when the number of items is sufficiently large or requires virtualization.
...
For styling, `data-side` is `"none"` on the `.Popup` and `.Positioner` parts when this positioning mode is active.
