...
The popup is unmounted automatically after its CSS closing animation finishes.
Once the trigger has been interacted with, the popup stays in the DOM and is hidden instead, so that its items remain available for typeahead on the closed trigger.

To control this step with an external animation library, call `eventDetails.preventUnmountOnClose()` in `onOpenChange` when `open` is `false`.
...
`onOpenChangeComplete(false)` fires when the popup is unmounted or hidden.
