...
Wrap `<Select.Root>` in `<Select.FilterProvider>`, add `<Select.FilterInput>` to the popup, and put the options inside `<Select.List>`.
The popup uses a dialog role to contain both the input and the listbox.

```jsx title="Filterable select"
<Select.FilterProvider>
  <Select.Root items={[{ value: 'fr', label: 'France' }]}>
    ...
    <Select.Portal>
      <Select.Positioner>
        <Select.Popup>
          ...
          <Select.FilterClear>Clear</Select.FilterClear>
          <Select.FilterEmpty>No results.</Select.FilterEmpty>
          <Select.List>
...
Keyboard and screen reader activation highlight an option while keeping focus on the input; pointer activation leaves the input highlighted.
By default, arrow keys move between the input and options.

The query resets on close. Multiple selects stay open after selection and keep the query.
Filtering or unmounting an option does not clear its selected value.
Use `items` or `itemToStringLabel` when option values need display labels.
`alignItemWithTrigger` is disabled for filterable selects.
...
Options match their text by default. Use `label` to override it and `keywords` to add search terms.
...
Empty groups are hidden. `<Select.FilterEmpty>` appears when no options match.

Use `filter` to replace the default case-insensitive substring matching.
It receives the trimmed query and each option's text or keywords, and keeps the option if any match.
...
`Select.useFilter` provides locale-aware string matching.

#### When to use Combobox

Use filterable Select for a short list of options.
Use [Combobox](/react/components/combobox) for large or async lists, virtualization, or an input outside the popup.
