...
Use [Filter Select](/react/components/filter-select) when users need to filter the options with a text input. It is a separate component with its own entrypoint, so an ordinary select never carries the filtering code.
