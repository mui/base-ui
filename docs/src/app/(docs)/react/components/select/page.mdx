...
When `alignItemWithTrigger` is active, Select suppresses anchor tracking by default with the `optimized` position update strategy. Set `updatePositionStrategy="always"` to track an animated or transformed trigger; this enables tracking even while `alignItemWithTrigger` is active. Explicitly setting `disableAnchorTracking` to `true` still disables tracking.
...
const actionsRef = React.useRef<Select.Root.Actions>(null);
