...
Import `FilterableSelect` from `@base-ui/react/filterable-select` to let users filter the available items. It exposes every ordinary select part plus `<FilterableSelect.Input>`, `<FilterableSelect.Clear>`, and `<FilterableSelect.Empty>`, so a filterable select needs only this one import.
...
Give `<FilterableSelect.Input>` its own accessible name with `aria-label` or a native `<label>`. `<FilterableSelect.Label>` labels the trigger, not the input.

Pass a function to `filter` to customize matching. It is called with the item's text and the trimmed query, and returns whether the item matches:

```jsx title="Custom filter"
<FilterableSelect.Root filter={(itemText, query) => itemText.startsWith(query)}>
```
