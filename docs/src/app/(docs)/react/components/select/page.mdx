...
A filterable select narrows `items` before the list renders, so only matching entries are mounted.
...
An ordinary select can still render `<Select.Item>` children directly. A filterable select requires `items`: filtering is a data operation, so the list must derive from data.
...
An entry can carry `keywords` to match additional terms beyond its label.
