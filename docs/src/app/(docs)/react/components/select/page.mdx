...
- **Prefer Combobox for large lists**: Use [FilterSelect](/react/components/filter-select) for a modest list of predefined values that needs filtering. Use [Combobox](/react/components/combobox) when the number of items is sufficiently large or requires virtualisation.
