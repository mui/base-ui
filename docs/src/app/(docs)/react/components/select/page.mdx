...
Use the `valueChangeAction` prop for value changes that perform async work. Select runs the action in a React transition and updates the displayed value optimistically while the action is pending.

See the [Async Actions guide](/react/handbook/async-actions) for details.

import { DemoSelectAsync } from './demos/async';
