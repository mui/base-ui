...
Use `<Select.Empty>` to display content when no `<Select.Item>` components are rendered.
The component checks the rendered items rather than the `items` prop on `<Select.Root>`.
Disabled items are still treated as rendered items, so they prevent the empty state from appearing.
Because the root element remains mounted, apply spacing and other layout styles to a child element.

import { DemoSelectEmpty } from './demos/empty';
