...
Options match their text by default. Use `label` to override it.

```jsx title="Labels"
<Select.Item value="nl" label="Netherlands">
...
It receives each option's text and the trimmed query, and keeps the option when it returns `true`.
