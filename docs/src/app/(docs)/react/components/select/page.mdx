...
- **Prefer Combobox for large lists**: Select can filter a modest list of predefined values. Use [Combobox](/react/components/combobox) when the number of items is sufficiently large or requires virtualisation.
- **Special positioning behavior**: The select popup overlaps its trigger by default so the selected item's text is aligned with the trigger's value text. This behavior [can be disabled or customized](/react/components/select#positioning).
...
It defaults to `true` for standard selects and `false` for filterable selects.
...
When set to `true`, there are a few important points to note about its behavior:
...
`items` is also the data source for the list. Pass a function as the `children` of `<Select.List>` to render one `<Select.Item>` per entry, the same way [Combobox](/react/components/combobox) does:

```jsx title="Rendering from items"
<Select.Root items={items}>
  <Select.List>
    {/* @highlight-start */}
    {(item) => (
...
Filtering then narrows `items` before the list renders, so only matching entries are mounted.

For grouped data, pass each group's entries to `<Select.Group>` and render them with `<Select.Collection>`:

```jsx title="Grouped items"
<Select.List>
  {(group) => (
...
Groups with no remaining matches are dropped.

Rendering `<Select.Item>` children yourself still works. Those items are matched on their rendered text instead.
...
Import `FilterSelect` from `@base-ui/react/filter-select` to let users filter the available items. It exposes every ordinary select part plus `<FilterSelect.Input>`, `<FilterSelect.Clear>`, and `<FilterSelect.Empty>`, so a filterable select needs only this one import.

Pass the items to the `items` prop and render them with a function as the `children` of `<FilterSelect.List>`. Filtering then narrows `items` before the list renders, so only matching entries are mounted:
...
import { DemoSelectFilterable } from './demos/filterable';
...
Pass a function to `filter` to customize matching. It is called with the item (the `items` entry, or the item's text when the list is authored as children) and the trimmed query, and returns whether the item matches:

```jsx title="Custom filter"
<FilterSelect.Root items={items} filter={(item, query) => item.startsWith(query)}>
```

The first argument is the item's `label` prop, falling back to its rendered text content. This differs from [Combobox](/react/components/combobox), which filters on item values.
