...
Pass the `filter` prop to `<Select.Root>` and add `<Select.Input>` to let users filter the available items.

import { DemoSelectFilterable } from './demos/filterable';

<DemoSelectFilterable />

The filter is fixed when the select mounts. To switch a select between filterable and non-filterable, give `<Select.Root>` a different `key` so it remounts.

Give `<Select.Input>` its own accessible name with `aria-label` or a native `<label>`. `<Select.Label>` labels the trigger, not the input.
...
<Select.Root filter={(itemText, query) => itemText.toLowerCase().startsWith(query.toLowerCase())}>
```
