...
| valueChangeAction    | `((value: Value[] \| Value \| null, eventDetails: Select.Root.ChangeEventDetails) => void \| PromiseLike<unknown>)` | -       | Action called when the value of the select changes. The action is run in a React transition when supported, and the selected&#xA;value is updated optimistically while the action is pending.                                                                                                                                                                                                                                                     |
