...
| filter               | `boolean \| SelectFilter`                                                                   | `false` | Enables filtering. Pass a function to customize how items match the query; it receives the&#xA;item's `label` (falling back to its rendered text) and the trimmed query. Read once when the select mounts. Give `Select.Root` a different `key` to switch a select&#xA;between filterable and non-filterable.                                                                                                                                     |
...
```typescript
type SelectRootInputValueChangeEventDetails = (
...
```typescript
type SelectRootInputValueChangeEventReason =
  | 'input-change'
  | 'input-clear'
  | 'clear-press'
  | 'popup-close';
...
- `Select.Root.InputValueChangeEventReason`: `SelectRootInputValueChangeEventReason`
- `Select.Root.InputValueChangeEventDetails`: `SelectRootInputValueChangeEventDetails`
