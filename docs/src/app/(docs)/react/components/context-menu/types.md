...
| actionsRef           | `React.RefObject<MenuRoot.Actions \| null>`                                    | -            | A ref to imperative actions. `unmount`: Manually unmounts the menu once its closing animation has finished.&#xA;Useful when the animation is controlled by an external JavaScript animation library.`close`: When specified, the menu can be closed imperatively.   |
