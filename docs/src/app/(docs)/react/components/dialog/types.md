...
| actionsRef              | `React.RefObject<Dialog.Root.Actions \| null>`                            | -       | A ref to imperative actions. `unmount`: Unmounts the dialog manually. Useful when the dialog's animation is controlled&#xA;by an external library: call `eventDetails.preventUnmountOnClose()` from `onOpenChange` to keep&#xA;the dialog mounted while it closes, then call `unmount` once the closing animation has finished.`close`: Closes the dialog imperatively when called.                                                                                                                                                                                                           |
