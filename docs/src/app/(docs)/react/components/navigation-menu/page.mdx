...
The popup unmounts automatically after its CSS closing animation finishes.

To control unmounting with an external animation library, call `eventDetails.preventUnmountOnClose()` in `onValueChange` when the value becomes `null`.
Then call `actionsRef.current.unmount()` after the animation finishes.
The opt-out applies to that close cycle and resets when the popup reopens.
To close the popup programmatically while keeping this control, call `actionsRef.current.close()` instead of setting `value` to `null` directly, so `onValueChange` still runs.
`onOpenChangeComplete(false)` fires when the popup unmounts.
See the [animation guide](/react/handbook/animation#manual-unmounting) for a complete example.
