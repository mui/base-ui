...
Use the `value` and `onValueChange` props to control which menu item is open. Setting `value` to `null` closes the popup, including when the update is triggered programmatically.

import { DemoNavigationMenuControlledClose } from './demos/controlled-close';

<DemoNavigationMenuControlledClose compact />
