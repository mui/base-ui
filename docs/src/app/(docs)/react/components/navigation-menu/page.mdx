...
Both `<NavigationMenu.Root>` (when it isn't nested) and `<NavigationMenu.Popup>` render a `<nav>` landmark. When both are present, give them distinct accessible names with `aria-label` (or `aria-labelledby`) so assistive technology can tell the two navigation regions apart.
