...
### Top-level links with menus

To keep top-level links navigable while preserving keyboard access to submenus, use `<NavigationMenu.TriggerGroup>` with a `<NavigationMenu.TriggerLink>` for navigation and a neighboring `<NavigationMenu.Trigger>` for disclosure.

import { DemoNavigationMenuTopLevelLinks } from './demos/top-level-links';

<DemoNavigationMenuTopLevelLinks compact />
...
<Reference
  ...
  parts="Root, List, Item, TriggerGroup, TriggerLink, Trigger, Icon, Content, Link, Backdrop, Portal, Positioner, Popup, Viewport, Arrow"
/>
