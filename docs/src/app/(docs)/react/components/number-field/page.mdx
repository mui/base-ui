...
Base UI proposes formatted text back through `onInputValueChange` when one is due — while typing, on blur, when stepping, and with the `'none'` reason when the formatted text is refreshed, either after an external `value`, `locale`, or `format` change or when typing ends and leftover unparseable text is reset.
Mirroring the callback back into `inputValue` reproduces the uncontrolled behavior; ignoring a given call keeps your own text instead. Note that declining every `'none'` proposal also suppresses the blur cleanup that clears leftover unparseable text.
