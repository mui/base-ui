...
The text itself is displayed as given — never formatted, never clamped — so any string is allowed.
...
Because the caller chose the string deliberately, the action is an escape hatch rather than a simulated keystroke:

- It applies even while the field is `disabled` or `readOnly`, which reject typing.
- It accepts characters typing would reject, so `'-'` works even when `min` is not negative.
- Changes it makes are final, not pending: `onValueChange` and `onValueCommitted` both run right away, with `reason: 'imperative-action'`. Waiting for blur would drop the change entirely when the input is never focused.
