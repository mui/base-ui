...
Use [Filter Menu](/react/components/filter-menu) when users need to filter the items with a text input. It is a separate component with its own entrypoint, so an ordinary menu never carries the filtering code.
