...
Only the provider imports the filtering code, so menus that don't filter don't bundle it.

Render `<Menu.FilterInput>` inside the popup and place the items inside `<Menu.List>`.
The popup becomes a dialog that contains the input and the list; every other part stays the same.

```jsx title="Filterable menu anatomy"
<Menu.FilterProvider>
  <Menu.Root>
    <Menu.Trigger />
    <Menu.Portal>
...
Wrap `<Menu.SubmenuRoot>` in its own `<Menu.FilterProvider>` to give a submenu a search field.
This works whether or not the parent filters.
A submenu doesn't inherit the parent's provider: an unwrapped submenu inside a filterable menu stays a plain menu, and its trigger still filters with the parent's query.

Hovering a submenu trigger opens the submenu without moving focus.
Focus moves to the submenu's input once the pointer enters the submenu, and back to the parent's input when the pointer returns.
Clicking the trigger or opening the submenu with the keyboard focuses its input right away.
