...
Wrap `<Menu.Root>` in `<Menu.FilterProvider>`, add `<Menu.FilterInput>` to the popup, and put the items inside `<Menu.List>`.
The popup uses a dialog role to contain both the input and the menu.
    ...
    <Menu.Portal>
      <Menu.Positioner>
        <Menu.Popup>
          ...
          <Menu.FilterClear>Clear</Menu.FilterClear>
...
Keyboard and screen reader activation highlight an item while keeping focus on the input; pointer activation leaves the input highlighted.
By default, arrow keys move between the input and items.
...
Items match their text by default. Use `label` to override it and `keywords` to add search terms.
...
Empty groups are hidden. `<Menu.FilterEmpty>` appears when no items match.

Use `filter` to replace the default case-insensitive substring matching.
It receives the trimmed query and each item's text or keywords, and keeps the item if any match.
...
`Menu.useFilter` provides locale-aware string matching.
...
Clicking or opening a submenu with the keyboard focuses its input.
Add `autoFocus` to `<Menu.FilterInput>` to also focus it when the submenu opens on hover.
