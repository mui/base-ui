...
Use the `indeterminate` prop when an item represents a mixed collection of settings. The selection state and parent-child relationships remain controlled by your application:

```jsx title="Controlled indeterminate state"
<Menu.CheckboxItem
  checked={allSelected}
...
The item reports `aria-checked="mixed"` while indeterminate. Set this state from your application when a related selection is partially complete. If the item controls other elements, add the appropriate `aria-controls` relationship.
