...
Import `FilterableMenu` from `@base-ui/react/filterable-menu` to let users filter the available items. It exposes every ordinary menu part plus `<FilterableMenu.Input>`, `<FilterableMenu.Clear>`, and `<FilterableMenu.Empty>`, so a filterable menu needs only this one import.
...
Filterability is scoped to the nearest root. A `<Menu.SubmenuRoot>` inside a filterable menu is not itself filterable; use `<FilterableMenu.SubmenuRoot>` for a submenu that filters.

Give `<FilterableMenu.Input>` an accessible name with `aria-label` or a native `<label>`.
...
<FilterableMenu.Root filter={(itemText, query) => itemText.startsWith(query)}>
```
