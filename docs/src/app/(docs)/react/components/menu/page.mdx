...
The filter parts are `<Menu.FilterInput>`, `<Menu.List>`, `<Menu.FilterClear>`, `<Menu.FilterEmpty>`, and `<Menu.FilterStatus>`; every other part stays the same.
A filterable popup is a dialog that contains the input and a `menu` list, so the items go inside `<Menu.List>`.
