...
The popup becomes a dialog that contains the input and the list, so the list is required here: an input can't sit inside a `menu` element.
...
A modal filterable menu (the default) keeps focus inside its popup like a dialog. Escape closes it, and a visually hidden close button lets screen reader users close it from their virtual cursor.
Pass `closeLabel` with a translated name for that button.
Set `modal={false}` on the root to let Tab leave the popup instead.
...
<Menu.FilterProvider filter={(text, query) => text.startsWith(query)} closeLabel="Dismiss menu">
