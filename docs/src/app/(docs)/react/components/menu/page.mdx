...
Add `autoFocus` to `<Menu.FilterInput>` to focus it whenever the menu opens.
Keyboard and screen reader activation highlight an item while keeping focus on the input.
