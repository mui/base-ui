...
A detached `<Menu.Trigger>` works with a filterable root through the same handle as a plain one.
It announces the filterable popup once the root attaches, which happens after hydration:

```jsx title="A detached trigger"
