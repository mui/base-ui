...
Wrap `<Menu.SubmenuRoot>` in its own `<Menu.FilterProvider>` to give it a search field.
Submenus don't inherit the parent's provider; an unwrapped submenu stays a plain menu.

Focus follows the pointer between the parent's input and the submenu's input.
Opening the submenu by click or keyboard focuses its input; add `autoFocus` to its `<Menu.FilterInput>` to focus it on hover as well.
