...
When the key changes, the viewport clears the highlight because the item list is rebuilt.
If remounting the content drops focus, focus moves to the popup.

The viewport does not infer a navigation direction.
This demo passes its own `data-view-direction` attribute based on the current view, then uses it with `[data-current][data-starting-style]` and `[data-previous][data-ending-style]` to slide the incoming and outgoing content.
