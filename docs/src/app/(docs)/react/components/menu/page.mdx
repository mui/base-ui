...
Pass the items to the `items` prop (required) and render them with a function as the `children` of `<FilterMenu.List>`. Filtering narrows `items` before the list renders, so only matching entries are mounted:
...
Entries can be strings or objects with a `label`, which is what filtering matches on. An object entry can carry anything else the render function needs, such as a nested list for a submenu, as in the demo above. An ordinary menu can still render `<Menu.Item>` children directly; a filterable menu requires `items`, since filtering is a data operation.
...
Pass a function to `filter` to customize matching. It receives the `items` entry and the trimmed query:
