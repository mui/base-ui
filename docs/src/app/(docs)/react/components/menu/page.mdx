...
Replace `<Menu.Root>` with `<Menu.FilterRoot>` to add a search field that filters the items.
The filter parts are `<Menu.FilterInput>`, `<Menu.FilterList>`, `<Menu.FilterClear>`, and `<Menu.FilterEmpty>`; every other part stays the same.
...
Only `<Menu.FilterRoot>` imports the filtering code, so menus that don't filter don't bundle it.
...
<Menu.FilterRoot filter={(text, query) => text.startsWith(query)}>
  {/* menu parts */}
</Menu.FilterRoot>
```
...
Combine `<Menu.FilterRoot>` with [Dialog](/react/components/dialog) to create a command palette.
...
Use `<Menu.FilterSubmenuRoot>` in place of `<Menu.SubmenuRoot>` to give a submenu its own search field, independently of whether the parent filters.
A plain `<Menu.SubmenuRoot>` inside a filterable menu keeps its ordinary menu semantics; its trigger still filters with the parent's query.
...
<Menu.FilterRoot handle={handle}>{/* menu parts */}</Menu.FilterRoot>
```
...
<TypesMenu.FilterRoot />

### FilterSubmenuRoot

<TypesMenu.FilterSubmenuRoot />
...
Use it when filtering items yourself with `filter={null}` on `<Menu.FilterRoot>`.
