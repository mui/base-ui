    ...
    <Menu.Portal>
      <Menu.Positioner>
        <Menu.Popup>
          ...
          <Menu.InputClear>Clear</Menu.InputClear>
...
Add `autoFocus` to `<Menu.Input>` to focus it whenever the menu opens.
...
Empty groups are hidden. `<Menu.Empty>` appears when no items match.
