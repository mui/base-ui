...
Filterability is scoped to the nearest root. A `<Menu.SubmenuRoot>` inside a filterable menu is not itself filterable; use `<FilterMenu.SubmenuRoot>` for a submenu that filters.
