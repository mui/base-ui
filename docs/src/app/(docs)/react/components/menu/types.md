...
type MenuRootActions = {
  unmount: () => void;
  close: () => void;
  focusItem: (target: Menu.Root.FocusItem) => void;
};
```
...
```typescript
type MenuRootFocusItem = 'first' | 'last' | 'none';
```
...
```typescript
function open(triggerId: string, focusItem?: Menu.Root.FocusItem | undefined): void;
```
...
- `Menu.Root`: `Menu.Root`, `Menu.Root.State`, `Menu.Root.Props`, `Menu.Root.Actions`, `Menu.Root.FocusItem`, `Menu.Root.ChangeEventReason`, `Menu.Root.ChangeEventDetails`, `Menu.Root.Orientation`
...
- `Menu.Root.FocusItem`: `MenuRootFocusItem`
