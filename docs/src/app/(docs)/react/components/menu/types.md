...
| delay        | `number`                                                                                   | `100`   | How long to wait before the menu may be opened on hover. Specified in milliseconds.&#xA;The delay is bypassed briefly after a committed hover close so quick&#xA;popup-to-trigger, trigger-to-trigger, and same-trigger re-entry handoffs&#xA;can reopen immediately. Requires the `openOnHover` prop. |
