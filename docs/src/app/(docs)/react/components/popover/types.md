...
| actionsRef            | `RefObject<Popover.Root.Actions \| null>`                                       | -         | A ref to imperative actions.`unmount`: When specified, the popover will not be unmounted when closed. Instead, the `unmount` function must be called to unmount the popover manually. Useful when the popover's animation is controlled by an external library., `close`: Closes the dialog imperatively when called.                                                                                                               |
