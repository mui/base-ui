...
Mounting a `DragAutoScroll.Root` enables auto-scroll for every scroll container on the page, like a provider does, and configures the one container it renders: the accepted drags, allowed axes, speed, or disabled state.
