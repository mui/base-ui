...
Like a provider, mounting `DragAutoScroll.Root` enables auto-scroll across the page. Its props configure the element it renders, including accepted drags, allowed axes, speed, and disabled state.
...
The `accept` prop determines the source payload type in callbacks such as `canScroll` and `allowedAxis`. The generated table uses `unknown` because it has no concrete type argument.
