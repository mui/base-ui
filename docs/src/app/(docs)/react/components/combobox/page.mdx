  ...
  <Combobox.Portal>
    ...
    <Combobox.Positioner>
      <Combobox.Popup>
        ...
        {/* Alternatively, use Virtualizer as the only child of List for large flat collections. */}
        <Combobox.List>
...
The virtualizer is the scroll container, so constrain its height or maximum height. The `--total-size` CSS variable contains the estimated or measured height of the virtual content and can be combined with `--available-height` from `<Combobox.Positioner>`:
