...
Use `getItemKey` to provide stable identity when item values are objects. `estimateSize` can be a number or a function receiving the item and its filtered index, while `overscanPx` controls the extra pixel buffer around the visible range. CSS `scroll-padding-top` and `scroll-padding-bottom` are respected when keyboard navigation scrolls an item into view.
