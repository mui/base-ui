...
With async data, prefer item objects as values so the selected label survives result changes, comparing them by ID with `isItemEqualToValue`. If IDs are required, keep the selected item in `data`, or label the ones that left it with `itemToStringLabel`: the collection labels the items it holds, and the prop covers the values it cannot resolve.
