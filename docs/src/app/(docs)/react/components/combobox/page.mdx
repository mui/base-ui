...
With async data, prefer item objects as values so the selected label survives result changes;
compare them by ID with `isItemEqualToValue`. If IDs are required, keep the selected item in `data`
or resolve every ID from a cache with `itemToStringLabel`, which replaces the collection's `label`
accessor.
