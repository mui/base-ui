...
Efficiently handle large datasets with `<Combobox.Virtualizer>`. It renders only the visible items while preserving Combobox keyboard navigation and selection behavior. Pass the collection to the `items` prop on `<Combobox.Root>`, and use the virtualizer as the only item-rendering child of `<Combobox.List>`.
