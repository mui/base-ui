  ...
  <Combobox.Portal>
    ...
    <Combobox.Positioner>
      <Combobox.Popup>
        ...
        <Combobox.List>
          <Combobox.Virtualizer estimateSize={32}>
...
Use `getItemKey` to provide stable identity when item values are objects or when TypeScript cannot infer the item type. For primitive values, you can instead declare the type explicitly, such as `<Combobox.Virtualizer<string>>`. `estimateSize` can be a number or a function receiving the item and its filtered index, while `overscanPx` controls the extra pixel buffer around the visible range (defaulting to the larger of 150px and the estimated item size). CSS `scroll-padding-top` and `scroll-padding-bottom` are respected when keyboard navigation scrolls an item into view.
