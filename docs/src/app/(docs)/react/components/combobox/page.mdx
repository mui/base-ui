...
Pass the collection to the `items` prop on `<Combobox.Root>`, and use the virtualizer as the only item-rendering child of `<Combobox.List>`. Each item renderer must return exactly one `<Combobox.Item>`. A grouped collection also needs a `renderGroupHeader` returning a `<Combobox.GroupLabel>`; see [grouped collections](/react/utils/virtualizer#grouped-collections). Grid mode is not currently supported.
...
Use `actionsRef.highlightItem()` to navigate the open list with custom keyboard shortcuts, as shown in [the Autocomplete example](/react/components/autocomplete#custom-keyboard-shortcuts).
