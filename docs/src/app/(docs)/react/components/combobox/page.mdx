...
To use a primitive value that differs from the source item (such as an ID held in an object), normalize the `items` array (flat or grouped) with the `Combobox.useItems` hook, which derives each item's value and label before rendering. Its derived value is authoritative, so omit the `value` prop from rendered items.
