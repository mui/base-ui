...
Use `itemToValue` to derive selection values automatically when rendering a collection. Mapped values should be defined and unique; `undefined` is normalized to `null` with a development warning. Collection items use the mapped value; manually rendered items must specify a `value` with the same type as the root's selected value.

For example, the source `items` can be user objects while selection uses only their `id`s. A `label` field is used automatically for display and filtering. For other shapes, `itemToStringLabel` receives the mapped value and the matching source item:
