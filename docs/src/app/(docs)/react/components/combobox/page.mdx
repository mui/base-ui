...
Use `createItems()` when rendering object items but storing primitive values such as IDs. Rendering and filtering receive the original objects, while selection, comparison, serialization, and `<Combobox.Item value>` use derived values that must be stable, unique, and non-null. For async or paginated data, include the selected item in `data` or provide `itemToStringLabel` until its record is available, and use object values only when the application intentionally wants the selected record as its public value.
