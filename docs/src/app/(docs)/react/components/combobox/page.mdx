...
- **Combobox is input-first**: The input is the control. The query persists while the user edits it, which suits asynchronous or external data, virtualized lists, autocomplete modes, and richer multiple-selection and chip composition.
- **Prefer a trigger-first control for discrete choices**: [Select](/react/components/select) is a form control with a trigger, and [FilterSelect](/react/components/filter-select) adds a transient finder inside its popup. Choose those for a fixed set of discrete choices in a modest, fully mounted list. Choose Combobox when the list is large, remote, or virtualized.
...
- **Avoid for actions**: Combobox selects a value. To let users find and run a command, use [FilterMenu](/react/components/filter-menu) instead.
- **Form controls must have an accessible name**: Label `<Combobox.Input>` with a native `<label>` or `<Field.Label>`, or provide an `aria-label` when no visible label is rendered. See the [forms guide](/react/handbook/forms).
...
In `multiple` mode, selecting an item clears the typed filter and closes the popup. To let users pick several results from one query, cancel the `item-press` close request in `onOpenChange`:

```jsx title="Keeping the popup open"
<Combobox.Root
  multiple
