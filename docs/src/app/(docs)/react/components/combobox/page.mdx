...
Combobox infers the selection value type from the `value`/`defaultValue` props on `<Combobox.Root>`.
If your `items` array contains objects with the shape `{ value, label }`, you can choose either of these patterns:
...
2. **Primitive selection**
   ...
   - `value`/`defaultValue` are the primitive `item.value` (string/number)
