...
Virtualization may not be necessary below a certain number of items: memoizing each item stops typing from re-rendering the whole list, which matters for the common case where items have rich children such as an indicator alongside a label. Wrap the item in [`React.memo`](https://react.dev/reference/react/memo) and pass the item as a prop — filtering preserves each item's reference, so unchanged options skip re-rendering:
