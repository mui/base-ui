...
Efficiently handle large datasets with `<Combobox.Virtualizer>`. It renders only the visible items while preserving Combobox keyboard navigation and selection behavior.
...
The `virtualized` prop on `<Combobox.Root>` is only needed when integrating a different virtualization library. Do not set it when using `<Combobox.Virtualizer>`.
