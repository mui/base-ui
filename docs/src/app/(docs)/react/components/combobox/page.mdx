...
Which request to cancel depends on the input's placement, and each callback has its own discriminator. When the input is outside the popup, cancel the close request in `onOpenChange`, identified by its `item-press` reason. When the input is inside, cancel the clear request in `onInputValueChange`: its reason stays `input-clear`, so selecting an item is marked with `eventDetails.isItemPress` instead.
...
The automatic clears that run when the popup closes or unmounts aren't marked, so they stay unaffected and the next open starts with a fresh filter.
