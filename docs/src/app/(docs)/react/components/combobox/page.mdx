...
Use `<Combobox.SelectAll>` as the first child of `<Combobox.List>` to add a list option that selects or deselects all currently filtered items. It only applies when `multiple` is set on `<Combobox.Root>`.

import { DemoComboboxSelectAll } from './demos/select-all';

<DemoComboboxSelectAll compact />
