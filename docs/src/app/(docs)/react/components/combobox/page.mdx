...
`createItems()` works well when object data is only needed by the Combobox. If controlled state is also rendered or otherwise consumed outside `<Combobox.Root>`, consider using the object itself as the value so the complete record remains available without a separate ID-to-item lookup.
