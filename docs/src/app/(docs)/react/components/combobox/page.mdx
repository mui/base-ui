...
An item has two roles: the source item that gets rendered, and the value that the application stores as the selection. With an ordinary `items` array these are the same object. The `Combobox.useItems` hook separates them, deriving a value from each source item — typically an ID held in that object:
...
| :----------------------------------------------------------------- | :----------------------------------------------------------------- |
| Function children of `<Combobox.List>` and `<Combobox.Collection>` | `value`, `defaultValue`, and `onValueChange`                       |
| `Combobox.useFilteredItems()`                                      | `isItemEqualToValue`, `itemToStringValue`, and `itemToStringLabel` |
| The `value` and `label` accessors of `Combobox.useItems`           | `onItemHighlighted` and the children of `<Combobox.Value>`         |
| The root `filter` function                                         |                                                                    |

When an item is rendered by the function child of `<Combobox.List>` or `<Combobox.Collection>`, the collection supplies its value, so the `value` prop on `<Combobox.Item>` can be omitted. The derived value is authoritative, and an explicit `value` prop on such an item is ignored. Items rendered manually outside a collection still require an explicit `value` prop set to the derived value.
