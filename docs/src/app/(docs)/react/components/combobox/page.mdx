...
With async data, prefer item objects as values so the selected label survives result changes, comparing them by ID with `isItemEqualToValue`. If IDs are required, the current selection keeps its label after its item leaves the filtered results. Use `itemToStringLabel` for values whose item never appeared at all, such as a selection restored from storage.
