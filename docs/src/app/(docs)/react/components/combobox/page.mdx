...
When a non-virtualized `items` array uses the full `{ value, label }` shape, `<Combobox.Item>` can use either the whole item or its `value` field.
Virtualized comboboxes do not support this primitive value inference.
