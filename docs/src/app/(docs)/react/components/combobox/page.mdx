...
When your app stores IDs rather than item objects, use `Combobox.useItems` to derive each item's selection value and display label. Selection props and events use the derived IDs, while list rendering continues to receive the original items. The label is also used for filtering and typeahead.
