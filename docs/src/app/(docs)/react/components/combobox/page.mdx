...
Which request to cancel depends on the input's placement. When the input is outside the popup, cancel the close request in `onOpenChange`; when it is inside, cancel the clear request in `onInputValueChange`.
