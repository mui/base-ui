...
For most large datasets, use the built-in `<Combobox.Virtualizer>`. It is the recommended approach because it renders only the visible items while handling Combobox keyboard navigation, scrolling, selection, and accessibility metadata for you.
...
<Combobox.Virtualizer
  trailing={loading ? <Spinner aria-hidden="true" /> : null}
...
You can instead integrate a third-party virtualization library when your application already uses one or needs behavior that the built-in virtualizer does not provide. The built-in `<Combobox.Virtualizer>` is recommended for most cases because a third-party integration must coordinate filtering, item indexes, scrolling, and accessibility metadata itself.
