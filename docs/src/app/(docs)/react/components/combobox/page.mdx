...
The selected value is therefore the item itself. When `items` holds objects, `value`, `defaultValue`, and `onValueChange` work with those objects rather than a field on them.
