...
When an item can be disabled, pass `isItemDisabled` to the root so a closed trigger can skip it during typeahead without rendering the popup. The predicate also disables the rendered item:

```jsx title="Disabled items"
<Combobox.Root items={fruits} isItemDisabled={(fruit) => fruit.disabled}>
  <Combobox.List>
    {(fruit) => (
...
With `createItems()`, the predicate receives the derived selection value, such as an ID. If you also set `disabled` on individual `<Combobox.Item>` elements, ensure the predicate returns `true` for the same values. Without the predicate, the popup mounts on the first matching key to read the items' `disabled` props.
