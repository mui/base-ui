...
For most large datasets, use the [`<ListVirtualizer>`](/react/utils/list-virtualizer) component. It is the recommended approach because it renders only the visible items while handling Combobox keyboard navigation, scrolling, selection, and accessibility metadata for you.
...
You can instead integrate a third-party virtualization library when your application already uses one or needs behavior that the built-in virtualizer does not provide. `<ListVirtualizer>` is recommended for most cases because a third-party integration must coordinate filtering, item indexes, scrolling, and accessibility metadata itself.
