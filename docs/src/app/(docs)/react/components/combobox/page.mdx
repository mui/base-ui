...
A collection created during render is a new collection every time, so it re-derives every value on each render and discards the labels it had resolved for items supplied through `filteredItems`.
