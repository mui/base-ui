...
Source items and explicitly specified `<Combobox.Item value>` props can have different types. Use `itemToValue` to derive selection values automatically when rendering a collection. Mapped values must be defined and unique. An explicit item `value` takes precedence over `itemToValue`.

For example, the source `items` can be user objects while selection uses only their `id`s. Their `label` fields are used automatically for display and filtering:
