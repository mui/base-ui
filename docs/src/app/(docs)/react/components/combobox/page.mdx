...
`Combobox.Separator` is a visual divider between options. It is presentational
inside the listbox and does not provide grouping semantics. Use
`Combobox.Group` and `Combobox.GroupLabel` to create meaningful groups of
options.
