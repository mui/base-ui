...
`<Combobox.InputGroup>` is optional. Use it when the input and adjacent parts should be treated as a single control for styling and accessibility. It gives the wrapper `role="group"`, keeps clicks on the wrapper from dismissing the popup, and lets the popup position against the whole control instead of only the input.
