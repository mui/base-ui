  ...
  <Combobox.Portal>
    ...
    <Combobox.Positioner>
      <Combobox.Popup>
        ...
        <Combobox.List>
          <Combobox.Virtualizer estimatedItemHeight={32}>
...
Use `getItemKey` to provide stable identity when item values are objects or when TypeScript cannot infer the item type. For primitive values, you can instead declare the type explicitly, such as `<Combobox.Virtualizer<string>>`. `estimatedItemHeight` can be a number or a function receiving the item and its filtered index, while `overscanPx` controls the extra pixel buffer around the visible range. It defaults to the larger of 150px and the first item's estimated height; even an explicit value of `0` keeps a minimum render buffer of one estimated row. CSS `scroll-padding-top` and `scroll-padding-bottom` are respected when keyboard navigation scrolls an item into view.
