...
Use the `valueMode` prop on `<Combobox.Root>` to control how item values are determined.

Use item mode (default) when each `<Combobox.Item>` receives the full item from the `items` array as its `value`.

1. **Item mode**
   - `<Combobox.Root valueMode="item">` and `<Combobox.Item value={item}>`
   - `value`/`defaultValue` are full items from `items` or `null`
   - When items are objects, they can use any shape
   - The normative object item shape is `{value, label}`; for custom shapes, use `itemToStringLabel` for input text and `itemToStringValue` for form serialization
   - If object references differ between `items` and root `value`, use `isItemEqualToValue` to compare stable fields such as `id`

Use value mode when each `<Combobox.Item>` receives a primitive `value` from object `items`.

2. **Value mode**
   - `<Combobox.Root valueMode="value">` and `<Combobox.Item value={item.value}>`
   - `value`/`defaultValue` are primitive values: `string`, `number`, or `null`, while `items` is an array of objects with shape `{value, label}`
...
>(props: Combobox.Root.Props<Value, Multiple, ValueMode>): React.JSX.Element {
