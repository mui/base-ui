...
Every change request caused by pressing an item is marked with `eventDetails.isItemPress`. Which one to cancel depends on the input's placement: cancel the close request in `onOpenChange` when the input is outside the popup, and the clear request in `onInputValueChange` when it is inside.
