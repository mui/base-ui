...
When your app stores IDs rather than item objects, use `Combobox.useItems` with `getValue` and `getLabel` to derive each item's selection value and display label. Selection props and events use the derived IDs, while list rendering continues to receive the original items. Pass the same derived ID to the `value` prop of `<Combobox.Item>`. The derived label is also used for filtering and typeahead.
