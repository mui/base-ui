...
Use `<Combobox.SelectAll>` inside `<Combobox.Popup>` to add a control that selects or deselects all currently filtered items. It only applies when `multiple` is set on `<Combobox.Root>`.
