...
const CHIP_LIMIT = 3;
const hiddenSelectionDescriptionId = 'hidden-selection-count';
    ...
    return (
      <>
        ...
        {hiddenCount > 0 && (
          <>
            {/* @highlight-start */}
            <span id={hiddenSelectionDescriptionId}>{`+${hiddenCount} more`}</span>
            {/* @highlight-end */}
          </>
        )}
        <Combobox.Input
          aria-describedby={hiddenCount > 0 ? hiddenSelectionDescriptionId : undefined}
        />
      </>
    );
  }}
