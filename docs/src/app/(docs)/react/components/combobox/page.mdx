...
When an item is rendered by the function child of `<Combobox.List>` or `<Combobox.Collection>`, the collection provides its value, so the `value` prop on `<Combobox.Item>` can be omitted. Ordinary collections use the source item as the value. Items rendered manually without a collection require an explicit `value` prop.

To use a primitive value that differs from the source item—such as an ID held in an object—normalize a flat `items` array with the `Combobox.useItems` hook, which derives each item's value and label before rendering. Its derived value is authoritative, so omit the `value` prop from rendered items.
...
Render callbacks and `useFilteredItems()` receive the source items, while selection props, change events, and the root `filter` function receive the derived values. Filtering remains configured on `<Combobox.Root>`: `itemToStringLabel` overrides the collection's label accessor, and `locale` and `limit` apply normally.

import { DemoComboboxUseItems } from './demos/use-items';

<DemoComboboxUseItems />

In React Server Components, use the hook-free `Combobox.items()` variant. It applies the accessors eagerly on the server and returns a serializable result. The source items and their derived values and labels must all be serializable. Pass that result through `useItems()` in the client component that renders the combobox:
