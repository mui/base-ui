...
| actionsRef           | `React.RefObject<Combobox.Root.Actions \| null>`                                                      | -       | A ref to imperative actions. `unmount`: Manually unmounts the combobox.&#xA;Passing this ref alone does not keep the popup mounted.&#xA;Call `preventUnmountOnClose()` in `onOpenChange` to manually control unmounting, then call this action after any externally controlled closing animation finishes. `close`: Closes the combobox imperatively when called.                                                                                                                                                                                                     |
