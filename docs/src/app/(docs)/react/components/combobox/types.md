...
| isItemDisabled       | `((itemValue: Value, index: number) => boolean)`                                                        | -       | Determines whether an item is disabled from its value and logical index for keyboard&#xA;navigation. Use this prop when disabled state must be known before an item is rendered, such as when&#xA;virtualizing the list. The `disabled` prop only marks a rendered item; this callback is what&#xA;makes keyboard navigation skip disabled items and gives rendered items their disabled state.                                                                                                                                                                       |
