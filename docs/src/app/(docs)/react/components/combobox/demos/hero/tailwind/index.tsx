    ...
    <Combobox.Root items={fruits}>
      <div className="relative flex flex-col gap-1 text-sm font-bold text-neutral-900">
        ...
        <Combobox.InputGroup className="relative box-content h-10 w-64 rounded-md border border-neutral-200 bg-[canvas] focus-within:outline-2 focus-within:-outline-offset-1 focus-within:outline-blue-800 [&>input]:pr-[calc(0.5rem+1.5rem)] has-[.combobox-clear]:[&>input]:pr-[calc(0.5rem+1.5rem*2)]">
          <Combobox.Input
            ...
            id={id}
            className="h-full w-full border-0 bg-transparent pl-3.5 text-base font-normal text-neutral-900 outline-none"
          />
      ...
      <Combobox.Portal>
        <Combobox.Positioner className="outline-none" sideOffset={4}>
          <Combobox.Popup className="w-[var(--anchor-width)] max-h-[23rem] max-w-[var(--available-width)] origin-[var(--transform-origin)] rounded-md bg-[canvas] text-neutral-900 shadow-lg shadow-neutral-200 outline-1 outline-neutral-200 transition-[transform,scale,opacity] data-[ending-style]:scale-95 data-[ending-style]:opacity-0 data-[starting-style]:scale-95 data-[starting-style]:opacity-0 dark:shadow-none dark:-outline-offset-1 dark:outline-neutral-300 duration-100">
            <Combobox.Empty>
              <div className="p-4 text-[0.925rem] leading-4 text-neutral-600">No fruits found.</div>
            </Combobox.Empty>
