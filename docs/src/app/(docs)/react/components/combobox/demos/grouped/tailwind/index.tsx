  ...
  return (
    <Combobox.Root items={groupedProduce}>
      <div className="relative flex flex-col gap-1 text-sm leading-5 font-bold text-neutral-950 dark:text-white">
        ...
        <Combobox.InputGroup className="relative h-8 w-64 border border-neutral-950 bg-white dark:bg-neutral-950 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-blue-800 dark:border-white [&>input]:pr-[calc(0.5rem+2rem)] has-[.combobox-clear]:[&>input]:pr-[calc(0.5rem+2rem*2)]">
          <Combobox.Input
      ...
      <Combobox.Portal>
        <Combobox.Positioner className="outline-none" sideOffset={4}>
          <Combobox.Popup className="w-[var(--anchor-width)] max-h-[23rem] max-w-[var(--available-width)] origin-[var(--transform-origin)] overflow-clip border border-neutral-950 bg-white text-neutral-950 shadow-[0.25rem_0.25rem_0_rgb(0_0_0_/_12%)] transition-[opacity,transform] duration-100 data-starting-style:scale-95 data-starting-style:opacity-0 data-ending-style:scale-95 data-ending-style:opacity-0 dark:border-white dark:bg-neutral-950 dark:text-white dark:shadow-none">
            ...
            <Combobox.List className="max-h-[min(23rem,var(--available-height))] overflow-auto overscroll-contain scroll-pt-[2.25rem] scroll-pb-2 outline-0">
              {(group: ProduceGroup) => (
                <Combobox.Group key={group.value} items={group.items} className="block pb-2">
                  <Combobox.GroupLabel className="sticky top-0 z-[1] mr-2 bg-white py-2 pr-2 pb-1 pl-7 text-xs leading-4 font-bold tracking-wider text-neutral-500 uppercase dark:bg-neutral-950 dark:text-neutral-400">
                    {group.value}
                  </Combobox.GroupLabel>
                  <Combobox.Collection>
                    {(item: Produce) => (
