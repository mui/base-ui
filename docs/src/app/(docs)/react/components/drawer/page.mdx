...
To enable swipe-to-open, render `<Drawer.SwipeArea />` and position it along the edge you want to accept gestures from. It listens for swipes in the opposite direction of `swipeDirection` by default. Use the `swipeDirection` prop to override and `swipeThreshold` to tune the minimum travel distance.
