...
- **Drawer extends [Dialog](/react/components/dialog):** It adds gesture support, snap points, and indent effect. If you don't need these, use Dialog instead. A panel that slides in from the edge of the screen and doesn't need gesture support is a positioned Dialog.
- **Best suited for mobile surfaces:** Bottom sheets, action sheets, and navigation panels where touch interactions are expected.
