...
Use `eventDetails.cancel()` in `onOpenChange` to prevent the drawer from closing when there are unsaved changes, and open a confirmation prompt instead.
...
import { DemoDrawerFormConfirmation } from './demos/form-confirmation';

<DemoDrawerFormConfirmation />
