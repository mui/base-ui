...
Use `<Drawer.VirtualKeyboardProvider>` when a drawer contains form fields and you want Base UI to manage visible viewport sizing and keyboard-aware focus handling for software keyboards. Existing drawers keep the legacy behavior by default.
...
Wrap a bottom-sheet drawer in `<Drawer.VirtualKeyboardProvider>` when it contains form controls and you want the drawer to react to software keyboards. This pattern keeps the main list scrollable while a footer field stays pinned outside the scroll container. Use `--available-height` to size the popup against the reduced visual viewport and `--drawer-keyboard-inset` to lift the sheet when the keyboard overlaps the bottom edge.
