...
Use `virtualKeyboardAware` when a drawer contains form fields and you want Base UI to manage visible viewport sizing and keyboard-aware focus handling for software keyboards. Existing drawers keep the legacy behavior by default.
...
Set `virtualKeyboardAware` when a bottom-sheet drawer contains form controls and you want the drawer to react to software keyboards. This pattern keeps the main list scrollable while a footer field stays pinned outside the scroll container.
