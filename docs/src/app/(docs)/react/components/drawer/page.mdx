...
Use `virtualKeyboardAware` when a drawer contains form fields on iOS and you want Base UI to manage visible viewport sizing and keyboard-aware focus handling for you. Existing drawers keep the legacy behavior by default.
...
Set `virtualKeyboardAware` when a bottom-sheet drawer contains form controls on iOS and you want the drawer to react to the software keyboard. This pattern keeps the main list scrollable while a footer field stays pinned outside the scroll container.
