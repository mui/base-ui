...
Pass `Drawer.virtualKeyboardAvoidance` to the `virtualKeyboardAvoidance` prop when a bottom sheet contains form fields and you want Base UI to manage keyboard-aware focus and scroll handling for software keyboards. It only applies when `swipeDirection` is set to `'down'`. Drawers without this prop are unaffected.
...
Pass `Drawer.virtualKeyboardAvoidance` to a bottom-sheet drawer when it contains form controls and you want the drawer to react to software keyboards. The avoidance strategy only applies when `swipeDirection` is set to `'down'`. Keep the popup frame stable, place header and footer content outside a plain scrollable body, and let the avoidance strategy scroll the focused field into the visible viewport when the keyboard opens.

```tsx title="Virtual keyboard aware drawer"
<Drawer.Root virtualKeyboardAvoidance={Drawer.virtualKeyboardAvoidance}>{/* ... */}</Drawer.Root>
```
