...
`<Drawer.Popup>` has `pointer-events: none`, while inner elements have `pointer-events: auto` so outside clicks still land on the backdrop and dismiss the drawer.
