...
Use `<Drawer.VirtualKeyboardProvider>` when a drawer contains form fields and you want Base UI to manage keyboard-aware focus and scroll handling for software keyboards. Existing drawers keep the legacy behavior by default.
...
Wrap a bottom-sheet drawer in `<Drawer.VirtualKeyboardProvider>` when it contains form controls and you want the drawer to react to software keyboards. Keep the popup frame stable, place header and footer content outside a plain scrollable body, and let the provider scroll the focused field into the visible viewport when the keyboard opens.
