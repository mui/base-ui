...
Wrap a bottom-sheet drawer in `<Drawer.VirtualKeyboardProvider>` when it contains form controls and you want the drawer to react to software keyboards. Keep the popup frame stable, place header and footer content outside a plain scrollable body, and let the provider scroll the focused field into the visible viewport when the keyboard opens. If a footer contains its own input, reserve a footer slot below the scroll area and lift the pinned footer with `--drawer-keyboard-inset`.
