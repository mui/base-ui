...
export default function ExampleDrawerSwipeArea() {
  ...
  return (
    <div
      ...
      className="relative min-h-[320px] w-full overflow-hidden border border-neutral-200 bg-neutral-50 text-neutral-900"
    >
      <Drawer.Root swipeDirection="right" modal={false}>
        ...
        <div className="flex min-h-[320px] flex-col items-center justify-center gap-3 px-4 text-center">
          <p className="text-base text-neutral-600 text-center pr-12">
        ...
        <Drawer.Portal container={portalContainer}>
          ...
          <Drawer.Viewport className="[--viewport-padding:0px] supports-[-webkit-touch-callout:none]:[--viewport-padding:0.625rem] absolute inset-0 z-20 flex items-stretch justify-end p-[var(--viewport-padding)]">
            <Drawer.Popup className="[--bleed:3rem] supports-[-webkit-touch-callout:none]:[--bleed:0px] h-full w-[calc(20rem+3rem)] max-w-[calc(100vw-3rem+3rem)] -mr-[3rem] bg-neutral-50 p-6 pr-[calc(1.5rem+3rem)] text-neutral-900 outline outline-1 outline-neutral-200 overflow-y-auto touch-auto [transform:translateX(var(--drawer-swipe-movement-x))] transition-transform duration-[450ms] ease-[cubic-bezier(0.32,0.72,0,1)] data-[swiping]:select-none data-[ending-style]:[transform:translateX(calc(100%-var(--bleed)+var(--viewport-padding)+2px))] data-[starting-style]:[transform:translateX(calc(100%-var(--bleed)+var(--viewport-padding)+2px))] data-[ending-style]:duration-[calc(var(--drawer-swipe-strength)*400ms)] supports-[-webkit-touch-callout:none]:mr-0 supports-[-webkit-touch-callout:none]:w-[20rem] supports-[-webkit-touch-callout:none]:max-w-[calc(100vw-20px)] supports-[-webkit-touch-callout:none]:rounded-[10px] supports-[-webkit-touch-callout:none]:pr-6 dark:outline-neutral-300">
              <Drawer.Content className="mx-auto w-full max-w-[32rem]">
                ...
                <Drawer.Description className="mb-6 text-base text-neutral-600">
