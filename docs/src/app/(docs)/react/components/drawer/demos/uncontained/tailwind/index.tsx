...
export default function ExampleDrawerUncontained() {
  ...
  return (
    <Drawer.Root open={open} onOpenChange={setOpen}>
      <Drawer.Trigger className="flex h-10 items-center justify-center rounded-md border border-neutral-200 bg-neutral-50 px-3.5 text-base font-normal text-neutral-900 select-none hover:bg-neutral-100 focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-800">
        Open action sheet
      </Drawer.Trigger>
      <Drawer.Portal>
        ...
        <Drawer.Viewport className="fixed inset-0 flex items-end justify-center">
          <Drawer.Popup className="box-border pointer-events-none flex w-full max-w-[28rem] flex-col gap-3 px-4 pb-[calc(1rem+env(safe-area-inset-bottom,0px))] outline-none focus-visible:outline-none [transform:translateY(var(--drawer-swipe-movement-y))] transition-transform duration-[450ms] ease-[cubic-bezier(0.32,0.72,0,1)] data-[swiping]:select-none data-[starting-style]:[transform:translateY(calc(100%+1rem+2px))] data-[ending-style]:[transform:translateY(calc(100%+1rem+2px))] data-[ending-style]:duration-[calc(var(--drawer-swipe-strength)*400ms)]">
            <Drawer.Content className="pointer-events-auto overflow-hidden rounded-2xl bg-neutral-50 text-neutral-900 outline outline-1 outline-neutral-200 dark:outline-neutral-300">
              ...
              <ul
                className="m-0 list-none divide-y divide-neutral-200 p-0"
              ...
              >
                {ACTIONS.map((action, index) => (
                  <li key={action}>
                    ...
                    <button
                      type="button"
                      className="block w-full border-0 bg-transparent px-5 py-4 text-center text-base text-neutral-900 select-none hover:bg-neutral-100 focus-visible:bg-neutral-100 focus-visible:outline-none"
            ...
            <div className="pointer-events-auto overflow-hidden rounded-2xl bg-neutral-50 outline outline-1 outline-neutral-200 dark:outline-neutral-300">
              <button
                type="button"
                className="block w-full border-0 bg-transparent px-5 py-4 text-center text-base text-red-700 select-none hover:bg-neutral-100 focus-visible:bg-neutral-100 focus-visible:outline-none"
                onClick={() => setOpen(false)}
              >
                Block User
              </button>
