...
export default function ExampleDrawerMobileNav() {
  ...
  return (
    <Drawer.Root swipeDirection="down">
      ...
      <Drawer.Portal>
        <Drawer.Backdrop className="fixed inset-0 min-h-[100dvh] bg-[linear-gradient(to_bottom,rgb(0_0_0/5%)_0,rgb(0_0_0/10%)_50%)] opacity-100 transition-[backdrop-filter,opacity] duration-[600ms] ease-[var(--ease-out-fast)] backdrop-blur-[1.5px] dark:opacity-70 supports-[-webkit-touch-callout:none]:absolute data-[starting-style]:opacity-0 data-[ending-style]:opacity-0 data-[starting-style]:backdrop-blur-0 data-[ending-style]:backdrop-blur-0 data-[ending-style]:duration-[250ms] data-[ending-style]:ease-[var(--ease-in-slow)]" />
        <Drawer.Viewport className="group fixed inset-0">
          <ScrollArea.Root
            style={{ position: undefined }}
            className="box-border h-full overscroll-contain transition-[transform,translate] duration-[525ms] ease-[cubic-bezier(0.45,1.005,0,1.005)] group-data-[starting-style]:translate-y-[100dvh] group-data-[ending-style]:pointer-events-none"
          >
            <ScrollArea.Viewport className="box-border h-full overscroll-contain">
              <ScrollArea.Content className="flex min-h-full items-end justify-center pt-8 md:py-16 md:px-16">
                <Drawer.Popup
                  ...
                  className="box-border w-full max-w-[42rem] outline-none transition-transform duration-[525ms] ease-[cubic-bezier(0.45,1.005,0,1.005)] translate-y-[var(--drawer-swipe-movement-y,0px)] data-[ending-style]:translate-y-[max(100dvh,100%)] data-[ending-style]:duration-[350ms] data-[ending-style]:ease-[cubic-bezier(0.375,0.015,0.545,0.455)]"
                >
                  <nav
                    aria-label="Navigation"
                    className="relative flex flex-col rounded-t-2xl bg-gray-50 px-6 pt-4 pb-6 text-gray-900 shadow-[0_10px_64px_-10px_rgb(36_40_52/20%),0_0.25px_0_1px_var(--color-gray-200)] outline outline-1 outline-gray-200 dark:outline-gray-300 dark:shadow-[0_0_0_1px_var(--color-gray-200)] md:rounded-xl"
                  >
