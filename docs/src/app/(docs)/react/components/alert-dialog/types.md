...
| actionsRef           | `React.RefObject<AlertDialog.Root.Actions \| null>`                            | -       | A ref to imperative actions. `unmount`: Unmounts the alert dialog manually. Useful when the alert dialog's animation is&#xA;controlled by an external library: call `eventDetails.preventUnmountOnClose()` from `onOpenChange`&#xA;to keep the alert dialog mounted while it closes, then call `unmount` once the closing animation&#xA;has finished.`close`: Closes the alert dialog imperatively when called. |
