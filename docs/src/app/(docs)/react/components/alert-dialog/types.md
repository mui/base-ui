...
| actionsRef           | `React.RefObject<AlertDialog.Root.Actions \| null>`                            | -       | A ref to imperative actions. `unmount`: Manually unmounts the alert dialog once its closing animation has finished.&#xA;Useful when the animation is controlled by an external JavaScript animation library.`close`: Closes the alert dialog imperatively when called. |
