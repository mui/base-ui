...
- **Use value-selection components for values**: Choose [FilterSelect](/react/components/filter-select) for a trigger-first control with a transient filter. Choose [Combobox](/react/components/combobox) when the input is the control or the results are large, virtualized, or loaded asynchronously.
