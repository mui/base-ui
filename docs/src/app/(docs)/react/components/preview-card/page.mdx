...
<Meta
  name="description"
  content="A high-quality, unstyled React preview card component for a link that shows a destination preview without interrupting keyboard or screen reader navigation."
/>
...
- **Protect screen reader users' current context**: Making every preview keyboard navigable would require screen reader users to move through all of its contents before reaching the next element on the current page. Repeatedly moving users out of their current context would be disruptive. Make sure the link remains the only accessible interface, and its destination contains the previewed information.
- **Keep popup content supplementary**: Avoid placing unique or essential information in the popup unless it is also available on the linked page. For the reason above, preview cards are a visual enhancement for sighted mouse and keyboard users and are not accessible to touch or screen reader users.
