...
Content can also change under a single trigger while the preview card stays open, such as when switching between tabs.
Pass a `transitionKey` to `<PreviewCard.Viewport>` and update it whenever the content changes.
When the key changes while the preview card is open, the viewport runs the same transition as a trigger switch and the popup animates to the new content's size.

import { DemoPreviewCardTransitionKey } from './demos/transition-key';

<DemoPreviewCardTransitionKey />

## API reference

import { TypesPreviewCard } from './types';

### Root

<TypesPreviewCard.Root />

### Trigger

<TypesPreviewCard.Trigger />

### Portal

<TypesPreviewCard.Portal />

### Backdrop

<TypesPreviewCard.Backdrop />

### Positioner

<TypesPreviewCard.Positioner />

### Popup

<TypesPreviewCard.Popup />

### Viewport

<TypesPreviewCard.Viewport />

Use the `Viewport` to animate the popup's content when it changes while open. When one popup is opened by several triggers, it morphs between each trigger's content automatically.

To animate content that changes in place under a single trigger, pass a `transitionKey` and update it whenever the content changes, such as `transitionKey={view}`. Either way, set `width: var(--positioner-width)` and `height: var(--positioner-height)` on the `Positioner` so its box stays fixed during the transition; without it, resizing can make the popup thrash or flip to another side.

### Arrow

<TypesPreviewCard.Arrow />

## createHandle

<TypesPreviewCard.createHandle />

[//]: # '@exclude-table-of-contents'

### Handle

<TypesPreviewCard.Handle />
