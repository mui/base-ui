...
- **Protect screen reader users' current context**: Making every preview screen reader navigable would require screen reader users to move through all of its contents before reaching the next element on the current page. Repeatedly moving users out of their current context would be disruptive. Make sure the link remains the only accessible interface, and its destination contains the previewed information.
- **Keep popup content supplementary**: Avoid placing unique or essential information in the popup unless it is also available on the linked page. For the reason above, preview card content is not keyboard or screen reader navigable and acts as a visual enhancement for sighted users only.
