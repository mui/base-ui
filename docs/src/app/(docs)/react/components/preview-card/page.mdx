...
- **Protect screen reader users' current context**: Making every preview available to screen readers would require users to move through all of its contents before reaching the next element on the current page. Repeatedly moving users out of their current context is disruptive. Make sure the link remains the only accessible interface and its destination contains the previewed information.
