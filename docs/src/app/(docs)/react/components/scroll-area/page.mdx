...
When the fade is applied to `<ScrollArea.Viewport>` itself, the variables can be used directly. Child elements still need to opt in with `inherit` if they use the overflow values.

```css title="scroll-area.module.css" {2-13}
.Viewport {
