...
To control unmounting with an external animation library, call `eventDetails.preventUnmountOnClose()` in `onOpenChange` when `open` is `false`.
...
To close the popup programmatically while keeping this control, call `actionsRef.current.close()` instead of setting `open` directly, so `onOpenChange` still runs.
