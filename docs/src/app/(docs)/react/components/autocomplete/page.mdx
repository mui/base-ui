...
Autocomplete infers the item type from the `items` prop passed to `<Autocomplete.Root>`. Always prefer passing the full item as item values (`<Autocomplete.Item value={item}>`), as the parameter passed to `itemToStringValue` is that inferred item type.
