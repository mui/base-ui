...
To derive a different value or label from each item, normalize the `items` array (flat or grouped) with the `Autocomplete.useItems` hook. Its derived value is authoritative, so omit the `value` prop from rendered items.

Render callbacks and `useFilteredItems()` receive the source items, while the root `filter` function receives the derived values. Filtering remains configured on `<Autocomplete.Root>`: `itemToStringLabel` overrides the collection's label accessor, and `locale` and `limit` apply normally.
