...
The action only applies while the popup is open. A call made in the same event as the one that opens the popup is dropped rather than queued, so a shortcut that should also reveal the list has to wait for the open to complete, for example by calling `highlightItem()` from `onOpenChangeComplete`. When `grid` is enabled, `'next'` and `'previous'` step through the items in DOM order, the same way <kbd>ArrowRight</kbd> and <kbd>ArrowLeft</kbd> do.
