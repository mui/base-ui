...
Pass a flat collection to the `items` prop on `<Autocomplete.Root>`, and use the virtualizer as the only item-rendering child of `<Autocomplete.List>`. Each item renderer must return exactly one `<Autocomplete.Item>`. Grouped collections and grid mode are not currently supported.
