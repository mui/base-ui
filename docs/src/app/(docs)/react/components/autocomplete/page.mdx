...
To derive a different value or label from each item, normalize a flat `items` array with the `Autocomplete.useItems` hook. Its derived value is authoritative, so omit the `value` prop from rendered items.
