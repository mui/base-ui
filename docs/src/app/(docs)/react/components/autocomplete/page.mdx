...
An item has two roles: the source item that gets rendered, and the value that the application stores as the selection. With an ordinary `items` array these are the same object. The `Autocomplete.useItems` hook separates them, deriving a value from each source item — typically an ID held in that object:

```jsx
const items = Autocomplete.useItems(suggestions, {
  value: getSuggestionId,
  label: getSuggestionName,
});
```

Every API sits on one side of that split:

| Receives the source item                                                   | Receives the value                                                 |
| :------------------------------------------------------------------------- | :----------------------------------------------------------------- |
| Function children of `<Autocomplete.List>` and `<Autocomplete.Collection>` | `value`, `defaultValue`, and `onValueChange`                       |
| `Autocomplete.useFilteredItems()`                                          | `isItemEqualToValue`, `itemToStringValue`, and `itemToStringLabel` |
| The `value` and `label` accessors of `Autocomplete.useItems`               | `onItemHighlighted` and the children of `<Autocomplete.Value>`     |
| The root `filter` function                                                 |                                                                    |

When an item is rendered by the function child of `<Autocomplete.List>` or `<Autocomplete.Collection>`, the collection supplies its value, so the `value` prop on `<Autocomplete.Item>` can be omitted. The derived value is authoritative, and an explicit `value` prop on such an item is ignored. Items rendered manually outside a collection still require an explicit `value` prop set to the derived value.

The `items` array may be flat or grouped, and the accessors always receive individual items rather than groups. Derived values should be stable, unique, and non-null primitives, because `null` and `undefined` mean no selection.

Filtering stays configured on `<Autocomplete.Root>`, where `itemToStringLabel` replaces the collection's `label` accessor rather than acting as a fallback behind it, while `locale` and `limit` apply normally. A custom `filter` receives the source item, so it can match against any of its fields, not only the derived value and label.
...
Normalizes items into a collection for the `items` prop of `<Autocomplete.Root>`, deriving each item's value and label before rendering.

<TypesAutocomplete.useItems hideDescription />
