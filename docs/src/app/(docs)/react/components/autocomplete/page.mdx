...
Passing `actionsRef` also makes unmounting the popup your responsibility: the built-in unmount-on-close is disabled whenever the prop is present, so call `unmount()` once the popup has finished closing.
