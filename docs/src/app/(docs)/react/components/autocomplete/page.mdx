...
The action only applies while the popup is open, so open it first if a shortcut should also reveal the list. When `grid` is enabled, `'next'` and `'previous'` do nothing, because a grid has no single item that follows the current one — use the arrow keys or `'first'` and `'last'` instead.
