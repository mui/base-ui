...
- **Translate the dismiss label**: Set `dismissButtonLabel` on `<Autocomplete.Root>` to label the visually hidden buttons that let screen reader users close the popup, for example `dismissButtonLabel="Fermer"` in French. The default is "Dismiss". The `locale` prop controls filtering and does not translate this label.
