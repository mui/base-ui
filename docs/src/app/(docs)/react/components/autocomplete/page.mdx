...
When an item is rendered by the function child of `<Autocomplete.List>` or `<Autocomplete.Collection>`, its source item is automatically used as its value, so the `value` prop on `<Autocomplete.Item>` can be omitted.
Items rendered manually without a collection require an explicit `value`.

To derive a different value or label from each item, normalize the `items` array with the `Autocomplete.useItems` hook (or its server-compatible `Autocomplete.items()` variant), which derives each item's value and label before rendering.
