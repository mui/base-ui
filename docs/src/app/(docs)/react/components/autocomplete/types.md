...
| autoHighlight        | `boolean \| 'always' \| ((itemValue: ItemValue, query: string) => boolean)`                                     | `false`  | Whether the first matching item is highlighted automatically. `true`: highlight after the user types and keep the highlight while the query changes.`'always'`: always highlight the first item.A function: called with the first matching item and the current query after the user&#xA;types; return `true` to highlight it or `false` to leave the highlight cleared.                                                                                                                                                                                                                                                           |
