...
Components provide data attributes designed for styling their states. For example, [Switch](/react/switch) can be styled using its `[data-checked]` and `[data-unchecked]` attributes, among others.
...
Components expose CSS variables to aid in styling, often containing dynamic numeric values to be used in sizing or transform calculations. For example, [Popover](/react/popover) exposes CSS variables on its `Popup` component like `--available-height` and `--anchor-width`.
