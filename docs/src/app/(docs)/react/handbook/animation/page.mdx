...
When using Motion, `opacity` animations are reflected in `element.getAnimations()`, so Base UI automatically waits for the animation finish before unmounting the component. If `opacity` shouldn't be animated (such as in a translating drawer component), you can use a value close to `1`, such as `opacity: 0.9999`.

[StackBlitz: Animating a Popover with Motion](https://stackblitz.com/edit/77tnrb42)

### Elements removed from the DOM when closed

Most components like Popover are unmounted from the DOM when they are closed. To animate them with Motion, follow these steps:
...
The `<Select>` component must be kept mounted in the DOM even when closed.
In this case, a different approach is needed to animate it with Motion.
