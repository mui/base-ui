...
For full control, you can manually unmount the component when it's closed once animations have finished.
Call `eventDetails.preventUnmountOnClose()` in `onOpenChange` when the component closes (in `onValueChange` when the value becomes `null` for Navigation Menu), then call `unmount()` on the `actionsRef` passed to the `<Root>` once the animation finishes.
`onOpenChangeComplete(false)` fires when the component unmounts.
