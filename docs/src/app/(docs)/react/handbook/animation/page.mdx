...
This example keeps the Select popup mounted with the Portal's `keepMounted` prop once it has been opened, so animating it with Motion uses a mix of the two previous approaches.
