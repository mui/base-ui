...
Dialog and Popover can animate with `<ViewTransition>` when `open` is controlled inside `startTransition`, the portal uses `keepMounted`, and the popup and its View Transition boundary are rendered conditionally from `open`.
