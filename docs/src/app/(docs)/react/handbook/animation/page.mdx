...
Components that specify `keepMounted` need a different approach to animate with Motion.
...
The Select component is initially unmounted before staying mounted after being interacted with. To animate it with Motion, a mix of the two previous approaches is needed.
