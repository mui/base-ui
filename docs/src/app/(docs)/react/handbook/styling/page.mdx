...
Import the part's data attribute and CSS variable constants from the same component entrypoint to get autocomplete when writing selectors and `var()` expressions.
The constants contain names such as `data-highlighted` and `--available-height`, rather than their current DOM or CSS values.

```tsx title="menu.tsx"
import { Menu, MenuItemDataAttributes, MenuPositionerCssVariables } from '@base-ui/react/menu';
