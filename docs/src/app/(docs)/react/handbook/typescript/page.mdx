...
- `ChangeEventDetails` (such as `Combobox.Root.ChangeEventDetails`) is the object passed to change handlers like `onValueChange`.
- `OpenChangeEventDetails` (such as `Combobox.Root.OpenChangeEventDetails`) is the object passed to `onOpenChange` on components whose `ChangeEventDetails` is shared with other handlers. It extends `ChangeEventDetails` with `preventUnmountOnClose()`.
...
function onOpenChange(open: boolean, eventDetails: Combobox.Root.OpenChangeEventDetails) {
