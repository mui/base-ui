...
<Description>
  {`Tooltips are visual-only floating elements that display information about a trigger element when a
  user hovers or focuses it.`}
</Description>
...
<Demo demo="UnstyledTooltipIntroduction" defaultCodeOpen="false" bg="gradient" />
