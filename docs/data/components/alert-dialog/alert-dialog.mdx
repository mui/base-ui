...
.AlertDialogPopup[data-alertdialog='open'] {
  opacity: 1;
...
- The open styles, placed on the base element class with `[data-alertdialog="open"]`
...
- `[data-alertdialog="open"]` - `open` state is `true`.
- `[data-alertdialog="closed"]` - `open` state is `false`. Can still be mounted to the DOM if closing.
