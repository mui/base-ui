...
<Callout type="info">
  `2 ** 31 - 1` is the maximum possible `z-index`, ensuring the popover appears on the very top
...
.PopoverPopup[data-popover='open'] {
  opacity: 1;
  transform: scale(1);
}
...
- The open styles, placed on the base element class with `[data-popover="open"]`
...
- `[data-popover="open"]` - `open` state is `true`.
- `[data-popover="closed"]` - `open` state is `false`. Can still be mounted to the DOM if closing.
