...
- `<Collapsible.Panel />` is component that contains the Collapsible's content
...
The dimensions of the `Content` subcomponent are provided as the `--collapsible-panel-height` and `--collapsible-panel-width` CSS variables
