...
Place the toggle button group in a HTML element or component with the `dir="rtl"` attribute to reverse directional keys for RTL languages:

```jsx
<html dir="rtl">
  <body>
    <ToggleButtonGroup.Root /> {/* RTL */}
  </body>
<html>
```

The `direction` prop can override this:

```jsx
<div dir="ltr">
  <ToggleButtonGroup.Root direction="rtl" /> {/* RTL */}
<div>
```
