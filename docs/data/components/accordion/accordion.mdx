...
The first one has an implicit `value` of `0`, the second one has a `value` of `1`, and so on.
...
- `[data-accordion="open"]` - `open` state is `true`.
- `[data-accordion="closed"]` - `open` state is `false`. Can still be mounted to the DOM if closing.
...
The dimensions of the `Accordion.Panel` subcomponent are provided as the `--accordion-panel-height` and `--accordion-panel-width` CSS variables.
