...
For instance, if the focus needs to stay inside of an [Menu](/components/react-menu/), then the component will be structured like this:
...
Press the **Open** button and then use the <kbd className="Kbd">Tab</kbd> key to move the focus—notice that it will not leave the Box:
...
If you leave the Box open in the demo, you won't be able to  on other buttons in this document.
...
The following demo uses the [Portal](/components/react-portal/) component to render a subset of the Focus Trap children into a new "subtree" outside of the current DOM hierarchy, so they are no longer part of the focus loop:
