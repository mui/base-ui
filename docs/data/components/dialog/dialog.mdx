...
- The open styles, placed on the base element class with `[data-dialog="open"]`
...
- `[data-dialog="open"]` - `open` state is `true`.
- `[data-dialog="closed"]` - `open` state is `false`. Can still be mounted to the DOM if closing.
