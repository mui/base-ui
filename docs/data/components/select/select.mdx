...
components: SelectRoot, SelectTrigger, SelectValue, SelectIcon, SelectBackdrop, SelectPositioner, SelectPopup, SelectItem, SelectItemText, SelectItemIndicator, SelectGroup, SelectGroupLabel, SelectScrollUpArrow, SelectScrollDownArrow, SelectArrow
...
By default, the Select is non-modal, so elements outside it can be interacted with.
To make it modal, use the `modal` prop.
You may also add a backdrop to the modal Select by using the `Select.Backdrop` component, to indicate that the outside elements are inert.
