...
.PreviewCardPopup[data-previewcard='open'] {
  opacity: 1;
  transform: scale(1);
}
...
- The open styles, placed on the base element class with `[data-previewcard="open"]`
...
- `[data-previewcard="open"]` - `open` state is `true`.
- `[data-previewcard="closed"]` - `open` state is `false`. Can still be mounted to the DOM if closing.
