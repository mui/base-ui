...
- `[data-starting-style]` - the popup was just inserted to the DOM. The attribute is removed 1 animation frame later. Enables "starting styles" upon insertion for conditional rendering.
- `[data-ending-style]` - the popup is in the process of being removed from the DOM, but is still mounted.
