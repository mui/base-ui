...
By default, the Menu is non-modal, so elements outside it can be interacted with.
To make it modal, use the `modal` prop.
You may also add a backdrop to the modal Menu by using the `Menu.Backdrop` component, to indicate that the outside elements are inert.
