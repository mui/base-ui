...
components: MenuItem, MenuPortal, MenuPositioner, MenuPopup, MenuRoot, MenuTrigger, MenuSubmenuTrigger, MenuArrow, MenuRadioGroup, MenuRadioItem, MenuRadioItemIndicator, MenuCheckboxItem, MenuCheckboxItemIndicator, MenuGroup, MenuGroupLabel, MenuBackdrop
...
- `<Menu.Portal />` renders the menu into a portal, allowing it to escape the bounds of its parent container.
...
The `side` prop also supports `inline-start` and `inline-end` as logical counterparts to `'left'` and `'right'`, which will automatically adapt to right-to-left automatically.
...
By default, the Menu is modal, so elements outside it cannot be interacted with.
To make it nonmodal, use the `modal` prop to `false`.

Modal menus may also use a backdrop by using the `Menu.Backdrop` component, to indicate that the outside elements are inert.
...
Additionally, be sure to use logical `side`s - `inline-start` and `inline-end` - for the `Positioner`.
