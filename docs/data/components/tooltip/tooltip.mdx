...
.TooltipPopup[data-tooltip='open'] {
  opacity: 1;
  transform: scale(1);
}
...
- The open styles, placed on the base element class with `[data-tooltip="open"]`
...
- `[data-tooltip="open"]` - `open` state is `true`.
- `[data-tooltip="closed"]` - `open` state is `false`. Can still be mounted to the DOM if closing.
