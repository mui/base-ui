...
export default function MenuIntroduction() {
  ...
  return (
    <div style={{ maxWidth: 800, padding: 20, margin: '20px auto' }}>
      <Menu.Root>
        ...
        <MenuPositioner>
          <MenuPopup>
            {items.map((item) => (
              <Menu.Root key={item}>
                <SubmenuTrigger label={item.toString()}>Item {item}</SubmenuTrigger>
                <MenuPositioner side="right" alignment="start">
                  <MenuPopup>
                    {items.map((subitem) => (
                      <MenuItem
                        key={subitem}
                        onClick={createHandleMenuClick(`${item}.${subitem}`)}
                        label={`${item}.${subitem}`}
                      >
                        Item {item}.{subitem}
                      </MenuItem>
