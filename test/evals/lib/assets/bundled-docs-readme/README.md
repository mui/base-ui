...
The installed package ships its full markdown documentation under
`node_modules/@base-ui/react/docs/react/`, so coding agents and IDEs can
consult the same reference as the website without network access:

- `docs/react/components/<component>.md` — per-component reference,
  including the canonical Anatomy block (the supported nesting of parts).
...
When assembling components, read the relevant `components/<name>.md`
first — composition rules, accessibility wiring, and behavior aren't
exhaustively expressible in the TypeScript types.
