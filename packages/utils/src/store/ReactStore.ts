...
import { useUntrackedCallback } from '../useUntrackedCallback';
...
> extends Store<State> {
  ...
  /**
   * Wraps a function with `useUntrackedCallback` to ensure it has a stable reference
  ...
  public useContextCallback<Key extends ContextFunctionKeys<Context>>(
    ...
    const stableFunction = useUntrackedCallback(fn ?? (NOOP as ContextFunction<Context, Key>));
