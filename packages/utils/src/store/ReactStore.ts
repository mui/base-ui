...
import { useUntracked } from '../useUntracked';
...
> extends Store<State> {
  ...
  /**
   * Wraps a function with `useUntracked` to ensure it has a stable reference
  ...
  public useContextCallback<Key extends ContextFunctionKeys<Context>>(
    ...
    const stableFunction = useUntracked(fn ?? (NOOP as ContextFunction<Context, Key>));
