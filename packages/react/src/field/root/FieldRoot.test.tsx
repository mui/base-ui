...
describe('<Field.Root />', () => {
  ...
  it.skipIf(isJSDOM)('sets `aria-labelledby` on Checkbox.Root during SSR before hydration', () => {
    renderToString(
      <Field.Root>
    ...
    expect(checkbox).to.have.attribute('aria-labelledby', label.id);
  });

  it.skipIf(isJSDOM)('sets `aria-labelledby` on Select.Trigger during SSR before hydration', () => {
    renderToString(
      <Field.Root>
        <Field.Label nativeLabel={false} render={<div />} data-testid="label">
          Label
  ...
  it.skipIf(isJSDOM)('sets `aria-labelledby` on Switch.Root during SSR before hydration', () => {
    renderToString(
      <Field.Root>
        <Field.Label nativeLabel={false} render={<div />} data-testid="label">
          Label
    ...
    expect(label.id).to.not.equal('');
    expect(switchEl).to.have.attribute('aria-labelledby', label.id);
  });

  it.skipIf(isJSDOM)('sets `aria-labelledby` on Radio.Root during SSR before hydration', () => {
    renderToString(
      <Field.Root>
        <Field.Label nativeLabel={false} render={<div />} data-testid="label">
          Label
    ...
    expect(label.id).to.not.equal('');
    expect(radio).to.have.attribute('aria-labelledby', label.id);
  });
