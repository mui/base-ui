...
describe('<Select.Popup />', () => {
  ...
  it.skipIf(isJSDOM)('keeps the first item aligned with the trigger after reopening', async () => {
    ...
    async function expectFirstItemAligned() {
      const value = screen.getByTestId('value');
      const firstItemText = screen.getByTestId('first-item-text');
        ...
        expect(
          Math.abs(
            getCenterY(value.getBoundingClientRect()) -
              getCenterY(firstItemText.getBoundingClientRect()),
          ),
        ).toBeLessThan(1);
      });
    }
