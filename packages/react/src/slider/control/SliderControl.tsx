...
export const SliderControl = React.forwardRef(function SliderControl(
  ...
  const setStylesRef = useUntracked((element: HTMLElement | null) => {
  ...
  const getFingerState = useUntracked((fingerCoords: Coords): FingerState | null => {
  ...
  const startPressing = useUntracked((fingerCoords: Coords) => {
  ...
  const focusThumb = useUntracked((thumbIndex: number) => {
    thumbRefs.current?.[thumbIndex]
      ?.querySelector<HTMLInputElement>('input[type="range"]')
      ?.focus({ preventScroll: true });
  });

  const handleTouchMove = useUntracked((nativeEvent: TouchEvent | PointerEvent) => {
  ...
  const handleTouchStart = useUntracked((nativeEvent: TouchEvent) => {
    if (disabled) {
      return;
    }
  ...
  const stopListening = useUntracked(() => {
    const doc = ownerDocument(controlRef.current);
