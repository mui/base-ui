...
export const SliderControl = React.forwardRef(function SliderControl(
  ...
  const setStylesRef = useUntrackedCallback((element: HTMLElement | null) => {
  ...
  const getFingerState = useUntrackedCallback((fingerCoords: Coords): FingerState | null => {
  ...
  const startPressing = useUntrackedCallback((fingerCoords: Coords) => {
  ...
  const focusThumb = useUntrackedCallback((thumbIndex: number) => {
    thumbRefs.current?.[thumbIndex]
      ?.querySelector<HTMLInputElement>('input[type="range"]')
      ?.focus({ preventScroll: true });
  });

  const handleTouchMove = useUntrackedCallback((nativeEvent: TouchEvent | PointerEvent) => {
  ...
  const handleTouchStart = useUntrackedCallback((nativeEvent: TouchEvent) => {
    if (disabled) {
      return;
    }
  ...
  const stopListening = useUntrackedCallback(() => {
    const doc = ownerDocument(controlRef.current);
