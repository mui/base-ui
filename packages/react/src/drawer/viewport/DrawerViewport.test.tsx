...
describe('<Drawer.Viewport />', () => {
  ...
  it.skipIf(isJSDOM)(
    'activates non-native interactive children on a real mouse click',
    async () => {
      ...
      await render(
        <Drawer.Root open>
          <Drawer.Portal>
            <Drawer.Viewport>
              <Drawer.Popup style={{ position: 'fixed', inset: 0 }}>
                <RadioGroup>
                  <Radio.Root value="a" style={{ display: 'block', width: 20, height: 20 }} />
                </RadioGroup>
              </Drawer.Popup>
            </Drawer.Viewport>
          </Drawer.Portal>
      ...
      // Trusted input, so the browser applies pointer capture's `click` retargeting.
