...
describe('<ListVirtualizer />', () => {
  ...
  describe('prop: trailing', () => {
    function TrailingList(props: { itemCount?: number; trailing?: React.ReactNode }) {
      return (
        <Combobox.Root defaultOpen items={createItems(props.itemCount ?? 100)}>
          <Combobox.List>
            <ListVirtualizer
              estimatedItemHeight={20}
              overscanPx={0}
    ...
    it.skipIf(isJSDOM)('sits after the last item and scrolls with it', async () => {
      ...
      const lastItem = screen.getByRole('option', { name: 'Item 100' });
    ...
    it('is not an item and does not appear in the collection metadata', async () => {
      ...
      expect(options[0]).to.have.attribute('aria-setsize', '3');

      // Rendered, but the list still contains only options.
      expect(screen.getByTestId('loading')).not.toBe(null);
      expect(screen.getByTestId('loading').closest('[role="option"]')).toBe(null);
    });
  });
