...
export interface DialogRootProps<Payload = unknown> {
   ...
   * - `unmount`: Unmounts the dialog manually. Useful when the dialog's animation is controlled
   * by an external library: call `eventDetails.preventUnmountOnClose()` from `onOpenChange` to keep
   * the dialog mounted while it closes, then call `unmount` once the closing animation has finished.
