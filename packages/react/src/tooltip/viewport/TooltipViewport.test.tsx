...
describe('<Tooltip.Viewport />', () => {
  ...
  describe('transition key', () => {
    ...
    it('should not set an activation direction when the anchor is unchanged', async () => {
      function TestComponent() {
        ...
        return (
          <React.Fragment>
            ...
            <Tooltip.Root open>
              <Tooltip.Trigger>Trigger</Tooltip.Trigger>
              <Tooltip.Portal>
                <Tooltip.Positioner anchor={() => anchorRef.current}>
                  <Tooltip.Popup>
                    <Tooltip.Viewport data-testid="viewport" transitionKey={step}>
                      <span data-testid={`content-${step}`}>Content {step}</span>
                    </Tooltip.Viewport>
                  </Tooltip.Popup>
                </Tooltip.Positioner>
    ...
    it.skipIf(isJSDOM)(
      'should derive the activation direction from an anchor change',
      async () => {
        function TestComponent() {
          ...
          return (
            <React.Fragment>
              <div
                ref={leftRef}
                style={{ position: 'fixed', left: 0, top: 0, width: 20, height: 20 }}
              />
              <div
                ref={rightRef}
                style={{ position: 'fixed', left: 200, top: 0, width: 20, height: 20 }}
              />
              <button type="button" data-testid="toggle" onClick={() => setStep((s) => 1 - s)}>
                toggle
              </button>
              <Tooltip.Root open>
                <Tooltip.Trigger>Trigger</Tooltip.Trigger>
                <Tooltip.Portal>
                  <Tooltip.Positioner anchor={step === 0 ? leftRef : rightRef}>
                    <Tooltip.Popup>
                      <Tooltip.Viewport data-testid="viewport" transitionKey={step}>
                        <span data-testid={`content-${step}`}>Content {step}</span>
                      </Tooltip.Viewport>
        ...
        expect(screen.getByTestId('viewport').getAttribute('data-activation-direction')).toContain(
          'right',
        );

        await user.click(screen.getByTestId('toggle'));
        await waitFor(() => {
          expect(screen.getByTestId('content-0')).toBeVisible();
        });
        expect(screen.getByTestId('viewport').getAttribute('data-activation-direction')).toContain(
          'left',
        );
