...
import type { getGridNavigatedIndex as getGridNavigatedIndexType } from '../utils/composite';
...
export type GridNavigator = typeof getGridNavigatedIndexType;
 ...
 */
export function useListNavigationCore(
  ...
  const commonOnKeyDown = useStableCallback((event: React.KeyboardEvent) => {
    ...
    // Grid navigation.
    if (gridNavigator && cols > 1) {
      const index = gridNavigator(listRef.current, {
        event,
        orientation,
        loopFocus,
        rtl,
        ...
        minIndex,
        maxIndex,
        prevIndex: indexRef.current > maxIndex ? minIndex : indexRef.current,
        stopEvent: true,
      });
