  ...
  React.useEffect(() => {
    ...
    function closeOnPressOutside(event: MouseEvent | PointerEvent | TouchEvent) {
      ...
      if (getOutsidePressEvent() === 'intentional') {
        ...
        // counts as an outside press. Press-less clicks — keyboard and
        // assistive technology activations as well as programmatic
        // `element.click()` — are still accepted. A click generated by a
        // physical press always reports a click count of at least 1, so
        // `detail === 0` identifies a press-less click even when the event
        // still carries a `pointerType` (Android assistive technology reports
        // one, and so do some synthetic clicks). `isVirtualClick` additionally
        // recognizes press-less clicks that do report a click count.
        if (
