...
export function useDismiss(
  ...
  const markInsideReactTree = useEventCallback((event: React.SyntheticEvent) => {
    insideReactTreeEventRef.current = event.nativeEvent;
    dataRef.current.insideReactTree = true;
  });

  const clearInsideReactTree = useEventCallback(() => {
    insideReactTreeEventRef.current = null;
    dataRef.current.insideReactTree = false;
  });

  const shouldSkipInsideReactTreeEvent = useEventCallback((event: Event) => {
