...
function App(
  inProps: Omit<Partial<UseListNavigationProps>, 'listRef'> & {
    ...
    onItemKeyDown?: React.KeyboardEventHandler<HTMLLIElement>;
  } = {},
) {
...
describe('useListNavigation', () => {
  it('forwards cross-axis keys from a virtual reference to the active item', async () => {
    const onItemKeyDown = vi.fn();
    render(<App virtual onItemKeyDown={onItemKeyDown} />);
    ...
    expect(onItemKeyDown).toHaveBeenCalledOnce();
    expect(onItemKeyDown.mock.calls[0][0]).toHaveProperty('key', 'ArrowRight');
  });
