...
const inertSelector = '[data-base-ui-inert]';
...
export interface UseDismissCoreProps {
  /**
 ...
 */
export function useDismissCore(
  context: FloatingRootContext | FloatingContext,
  props: UseDismissCoreProps = {},
): ElementProps {
  const {
    enabled = true,
    escapeKey = true,
  ...
  const outsidePressFn = useStableCallback(
    typeof outsidePressProp === 'function' ? outsidePressProp : undefined,
  );
  const outsidePress = typeof outsidePressProp === 'function' ? outsidePressFn : outsidePressProp;
  const outsidePressEnabled = outsidePress !== false;
  const getOutsidePressEventProp = useStableCallback(() => outsidePressEvent);

  const escapeKeyBubbles = typeof bubbles === 'boolean' ? bubbles : (bubbles?.escapeKey ?? false);
  ...
  React.useEffect(() => {
    ...
    function getOutsidePressEvent(): PressType {
      ...
      return resolved[type === 'touch' ? 'touch' : 'mouse'];
    }
    ...
    function closeOnPressOutside(event: MouseEvent | PointerEvent | TouchEvent) {
      ...
      if (computedOutsidePressEvent === 'intentional' && suppressNextOutsideClickRef.current) {
    ...
    function addTargetEventListenerOnce<EventType extends Event>(
      ...
      addEventListener(target, event.type, listener as EventListener, { once: true });
    }
    ...
    function closeOnPressOutsideCapture(event: PointerEvent | MouseEvent) {
      ...
      addTargetEventListenerOnce(
        event,
        (event.type === 'pointerdown' ? handlePointerDown : closeOnPressOutside) as (
