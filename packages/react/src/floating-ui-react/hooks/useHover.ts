...
export function useHover(
  ...
  const handleCloseRef = useUntrackedRef(handleClose);
  const delayRef = useUntrackedRef(delay);
  const openRef = useUntrackedRef(open);
  const restMsRef = useUntrackedRef(restMs);
  ...
  const isHoverOpen = useUntrackedCallback(() => {
  ...
  const cleanupMouseMoveHandler = useUntrackedCallback(() => {
    unbindMouseMoveRef.current();
    handlerRef.current = undefined;
  });

  const clearPointerEvents = useUntrackedCallback(() => {
  ...
  const isClickLikeOpenEvent = useUntrackedCallback(() => {
