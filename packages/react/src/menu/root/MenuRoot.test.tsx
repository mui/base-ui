...
// Mainline tests assert the non-VoiceOver behavior deterministically on every host OS
// (`platform.screenReader.voiceOver` is true on any Apple platform, which would hide filterable
// lists from role queries). `MenuList.voiceOver.test.tsx` covers the VoiceOver-specific path.
...
describe('<Menu.Root />', () => {
  ...
  describe('filtering', () => {
    ...
    it.each([
      ['Input', FilterMenu.Input],
      ['Clear', FilterMenu.Clear],
      ['Empty', FilterMenu.Empty],
    ] as const)(
      'throws a scoped error when FilterMenu.%s is used inside an ordinary Menu.Root',
      async (name, Part) => {
        ...
        try {
          await expect(
            ...
            `Base UI: <FilterMenu.${name}> must be placed within the nearest ` +
    ...
    it('scopes filtering to the nearest filterable root', async () => {
      ...
      try {
        await expect(
          ...
          'Base UI: <FilterMenu.Input> must be placed within the nearest ' +
            '<FilterMenu.Root> or <FilterMenu.SubmenuRoot>, imported from ' +
            '`@base-ui/react/filter-menu`. An ordinary <Menu.Root> cannot filter.',
        );
      } finally {
