...
describe('<Menu.Root />', () => {
  ...
  describe.for([
    ...
    describe.skipIf(isJSDOM)('mouse interaction', () => {
      ...
      it('keeps a dialog opened by an item open when the drag-release gesture emits a trailing click', async () => {
        ignoreActWarnings();
          ...
          return (
            <div>
              ...
              <Dialog.Root
                open={dialogOpen}
                onOpenChange={(open, details) => {
                  dialogOpenChangeSpy(open, details.reason);
                  setDialogOpen(open);
                }}
              >
                <Dialog.Portal>
        ...
        fireEvent.mouseUp(screen.getByRole('menuitem', { name: 'Open dialog' }));

        await waitFor(() => {
          expect(screen.queryByTestId('dialog-popup')).not.toBe(null);
        });

        // When the press starts on the trigger and is released over the
        // portalled item, the browser fires the gesture's native click on
        // their nearest common ancestor (the body) after the dialog has
        // already opened. That click must not count as an outside press.
        ...
        // A genuine outside press must still dismiss the dialog.
        ...
        expect(dialogOpenChangeSpy).toHaveBeenCalledWith(false, REASONS.outsidePress);
      });
    });
