...
import { findScrollableTouchTarget } from '../drawer/popup/DrawerPopup';
...
export function useSwipeDismiss(options: useSwipeDismiss.Options): useSwipeDismiss.ReturnValue {
  ...
  const handlePointerDown = useStableCallback((event: React.PointerEvent) => {
    ...
    if (event.pointerType === 'touch' && findScrollableTouchTarget(target, document.body)) {
      return;
    }
    ...
    if (ignoreScrollableAncestors && event.pointerType === 'touch' && element && target) {
