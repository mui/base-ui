...
/**
 * Plugin to build the props to pass to the section list element.
 */
export class TemporalFieldSectionListPropsPlugin<TValue extends TemporalSupportedValue, TError> {
  private store: TemporalFieldStore<TValue, any, TError>;

  // We can't type `store`, otherwise we get the following TS error:
  // 'sectionListProps' implicitly has type 'any' because it does not have a type annotation and is referenced directly or indirectly in its own initializer.
  ...
  public handleKeyDown = (event: React.KeyboardEvent<HTMLSpanElement>) => {
    ...
    // This component should not be rendered when all sections are selected.
    // And when it is focused, we should always have a selected section.
    if (selectedSections === 'all' || selectedSections == null) {
      return;
    }
    ...
    switch (true) {
      ...
      case this.store.valueAdjustment.isAdjustSectionValueKeyCode(event.key): {
        ...
        if (selectors.readOnly(this.store.state) || activeSection == null) {
          break;
        }
