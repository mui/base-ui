 ...
 */
export function createChangeEventDetails<Reason extends string, Additional extends object = {}>(
  reason: Reason,
  event?: ReasonToEvent<Reason>,
  additional?: Additional,
): BaseUIChangeEventDetails<Reason, Additional> {
  let canceled = false;
  let allowPropagation = false;
  const resolvedAdditional = additional ?? (EMPTY_OBJECT as Additional);
  return {
    reason,
...
>(
  ...
  event?: EventType,
  additional?: Additional,
): BaseUIGenericEventDetails<Reason, EventType, Additional> {
  const resolvedAdditional = additional ?? (EMPTY_OBJECT as Additional);
  return {
    reason,
