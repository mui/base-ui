/* eslint-disable @typescript-eslint/no-shadow, no-plusplus, default-case, no-underscore-dangle, consistent-return, no-nested-ternary */
import { isElement } from '@floating-ui/utils/dom';
import type { HandleClose } from '@floating-ui/react';
import { contains, getTarget, getNodeChildren } from '@floating-ui/react/utils';
import type { Rect, Side } from '@floating-ui/utils';
...
export interface SafePolygonOptions {
  ...
  blockClose?: (event: MouseEvent) => boolean;
}
...
export function safePolygon(options: SafePolygonOptions = {}) {
  const { buffer = 0.5, blockPointerEvents = false, requireIntent = true, blockClose } = options;
