...
export const LabelableProvider: React.FC<LabelableProvider.Props> = function LabelableProvider(
  ...
  const defaultLabelId =
    props.fromControlId && initialControlId ? `${initialControlId}-label` : undefined;

  const [controlId, setControlIdState] = React.useState<string | null | undefined>(
    initialControlId,
  );
  const [labelId, setLabelId] = React.useState<string | undefined>(props.labelId ?? defaultLabelId);
...
export interface LabelableProviderProps {
  ...
  fromControlId?: boolean | undefined;
