...
/**
 * Resolves the value rendered for a registry slot. Mounted items register their exact
 * `<Combobox.Item value>` (`renderedValue`); for a slot no mounted item registered, fall
 * back to the item itself (virtualized) or its inferred value (non-virtualized) so callers
 * never hand the outer `{ value, label }` item to a user stringifier or comparator.
 */
export function resolveRenderedValue(
  renderedValue: any,
  candidate: any,
  virtualized: boolean,
): any {
  ...
  return virtualized ? candidate : inferItemValue(candidate);
}
