...
export const ScrollAreaRoot = React.forwardRef(function ScrollAreaRoot(
  ...
  const handleScroll = useUntrackedCallback((scrollPosition: { x: number; y: number }) => {
  ...
  const handlePointerDown = useUntrackedCallback((event: React.PointerEvent) => {
    if (event.button !== 0) {
      return;
    }
  ...
  const handlePointerMove = useUntrackedCallback((event: React.PointerEvent) => {
  ...
  const handlePointerUp = useUntrackedCallback((event: React.PointerEvent) => {
