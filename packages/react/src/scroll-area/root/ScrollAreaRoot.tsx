...
export const ScrollAreaRoot = React.forwardRef(function ScrollAreaRoot(
  ...
  const handleScroll = useUntracked((scrollPosition: { x: number; y: number }) => {
  ...
  const handlePointerDown = useUntracked((event: React.PointerEvent) => {
    if (event.button !== 0) {
      return;
    }
  ...
  const handlePointerMove = useUntracked((event: React.PointerEvent) => {
  ...
  const handlePointerUp = useUntracked((event: React.PointerEvent) => {
