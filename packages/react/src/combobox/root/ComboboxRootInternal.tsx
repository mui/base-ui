  ...
  const setSelectedValue = useEventCallback(
    (nextValue: Value | Value[], eventDetails: ComboboxRootInternal.ChangeEventDetails) => {
      ...
      if (multiple && hadInputValue) {
        setInputValue('', createChangeEventDetails(eventDetails.reason, eventDetails.event));
...
interface ComboboxRootProps<ItemValue> {
   ...
   * Receives the highlighted value (or `undefined` when none) and event details
   * describing the navigation reason and highlighted index.
   */
  onItemHighlighted?: (
