...
describe('<Combobox.Status />', () => {
  ...
  it('renders only when open', async () => {
    ...
    expect(screen.queryByTestId('status')).to.equal(null);
    await user.click(screen.getByTestId('input'));
    await waitFor(() => expect(screen.getByTestId('status')).not.to.equal(null));
  });
  ...
  describe('a11y', () => {
    ...
    it('removes the initial text mutation one tick after mount', async () => {
      ...
      expect(screen.getByRole('status')).to.equal(screen.getByTestId('status'));
      expect(screen.getByTestId('status').textContent).to.equal('Searching…\u2060');
    ...
    it('updates content immediately after the live region has mounted', async () => {
      ...
      expect(screen.getByTestId('status')).to.have.text('Searching…');
    });
    ...
    it('preserves a custom render prop on the visible element', async () => {
      ...
      expect(screen.getByTestId('custom-status').tagName).to.equal('P');
      expect(screen.getByRole('status')).to.equal(screen.getByTestId('custom-status'));
      expect(screen.getByTestId('custom-status').textContent).to.equal('Searching…\u2060');
