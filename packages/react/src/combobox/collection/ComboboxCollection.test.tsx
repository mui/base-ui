...
describe('<Combobox.Collection />', () => {
  ...
  it('still warns when a collection child has no key', async () => {
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});

    try {
      await render(
        <Combobox.Root items={['alpha', 'beta']} defaultOpen>
          <Combobox.Input />
          <Combobox.List>{(item: string) => <Combobox.Item>{item}</Combobox.Item>}</Combobox.List>
        </Combobox.Root>,
      );

      expect(
        errorSpy.mock.calls.some((call) => String(call[0]).includes('unique "key" prop')),
      ).toBe(true);
    } finally {
